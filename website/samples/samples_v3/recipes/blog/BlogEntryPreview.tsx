        ...
        <div>
            ...
            <Container className={"mb-16"}>
                <Container maxWidth={"3xl"}>
                    {modifiedValues?.name && (
                        <Typography variant={"h3"} className="mt-16 mb-12 mx-12">
                            {modifiedValues.name}
                        </Typography>
                    )}
                </Container>
    ...
    return (
        <div className="flex justify-center">
            {storagePaths.map((path, index) => (
                <div key={`images_${index}`} className="m-4 p-8 w-[350px] h-[350px]">
...
export function StorageImage({ storagePath }: { storagePath: string }) {
    ...
    useEffect(() => {
        if (storagePath) {
            storage.getDownloadURL(storagePath).then((res) => setUrl(res.url ?? undefined));
        }
    }, [storage, storagePath]);
...
function Quote({ quoteText }: { quoteText: string }) {
    ...
    return (
        <Container
            maxWidth={"5xl"}
            className={"border-l-2 border-l-red-950 dark:border-l-red-100 my-8 italic"}
        >
