...
export function DataTalkIntro(): JSX.Element {
    return (
        <Panel color={"primary"} includePadding={false}>
...
function NicerTable() {
    return (
        <div className="h-full w-full flex flex-col bg-surface-950 text-text-primary-dark rounded-lg shadow-sm overflow-hidden border border-surface-800 border-opacity-40">
            {/* Header Toolbar Section */}
            <div className="border-surface-700 border-opacity-40 no-scrollbar min-h-[56px] overflow-x-auto px-2 md:px-4 bg-surface-900 border-b flex flex-row justify-between w-full flex-shrink-0">
                {/* Left Side: Title */}
                <div className="flex gap-2 md:mr-4 mr-2 self-stretch items-center">
                    <div className="hidden lg:block">
                        <div className="flex flex-col items-start justify-center">
                            <h6 className="text-base font-medium leading-none truncate max-w-[160px] lg:max-w-[240px]">
                                Products
                            </h6>
                            <div className="text-xs text-text-secondary-dark w-full text-ellipsis block overflow-hidden whitespace-nowrap max-w-xs text-left">
                                {productsData.length} entities
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                    <div className="relative h-[42px] bg-surface-800 border border-surface-700 border-opacity-40 rounded-lg flex items-center">
                        <div className="absolute p-0 px-4 h-full pointer-events-none flex items-center justify-center top-0 left-0">
                            <span className="material-icons block select-none text-text-disabled-dark text-2xl">
                                search
                            </span>
                        </div>
                        <input
            ...
            <div className="flex-grow overflow-auto">
                {/* Sticky Header Row */}
                <div className="sticky top-0 z-10 flex flex-row w-fit min-w-full border-b border-surface-700 border-opacity-40 bg-surface-900 h-12">
                    {/* Header Cell: ID */}
                    <div
                    ...
                    >
                        ...
                        <div className="absolute h-full w-[6px] top-0 right-0 cursor-col-resize bg-transparent hover:bg-gray-600 opacity-50"></div>
                    </div>
                    {/* Header Cell: Name */}
                    <div
                        className="flex py-0 px-3 h-full text-xs uppercase font-semibold select-none items-center text-text-secondary-dark hover:text-text-primary-dark hover:bg-surface-800 hover:bg-opacity-50 relative"
                    ...
                    >
                        <div className="overflow-hidden flex-grow flex items-center justify-start">
                            <span
                                className="material-icons block select-none mr-1 text-xl"
                                style={{ fontSize: "20px" }}
                            >
                                short_text
                            </span>
                    ...
                    <div className="p-0.5 w-20 h-full flex items-center justify-center cursor-pointer bg-surface-950 bg-opacity-40 hover:bg-surface-950 flex-shrink-0">
                        <span
                            className="material-icons block select-none text-xl"
                            style={{ fontSize: "20px" }}
                        >
                            add
                        </span>
                    ...
                    {productsData.map((row, index) => (
                        ...
                        >
                            ...
                            <div
                                ...
                                style={{ width: "130px" }}
                            >
                                <div className="w-full overflow-hidden truncate font-mono text-xs text-text-secondary-dark text-center px-1">
                                    {row[0]}
                                </div>
                            </div>
                            {/* Cell: Name */}
