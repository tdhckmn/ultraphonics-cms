...
export function VersionsComparison() {
    function getFeatureComponent(value: string) {
        if (value === "Enterprise")
            return (
                <a className="text-inherit" href={useBaseUrl("enterprise/")}>
                    Enterprise
                </a>
            );
        if (value === "Yes") return <CheckIcon className="m-auto" width={24} />;
        if (value === "No") return <RemoveIcon className="m-auto" width={24} />;
        if (value === "WIP") return <ScheduleIcon className="m-auto" width={24} />;
        return value;
    }
        ...
        <>
                ...
                header={
                    <>
                        ...
                        <div className={clsx("border-0 border-b w-full", defaultBorderMixin)}>
                            <h2
                                className={"uppercase font-mono my-8 text-center"}
                                id={"comparison"}
                            >
                                Versions comparison
                            </h2>
            ...
            >
                <div className={ContainerInnerPaddingMixin}>
                    <table className="table font-medium border-separate text-sm text-left mt-4 mb-8 mx-auto">
                        {/*<thead*/}
                        ...
                        <thead className="text-sm text-gray-800 uppercase font-mono bg-gray-50 font-bold">
                            <tr>
                                <th
                                    scope="col"
                        ...
                        <tbody>
                            {data.map((row, index) => (
                                <tr key={index} className="border-b ">
                                    ...
                                    <td
                                        className={
                                            "bg-gray-50 mx-2  border-none rounded-lg px-4 py-2 text-gray-800  text-center " +
                                            getEntryClass(row.community)
                                        }
                                    >
                        ...
                        <tfoot className="text-xs text-gray-800 uppercase font-mono bg-gray-50  font-bold">
                            <tr>
                                <th
                                    scope="col"
                                    colSpan={2}
                                ...
                                ></th>
                                <th className={" table-cell p-0 border-none"} colSpan={1}>
                                    <a
                                        className={CTAOutlinedButtonMixin + " w-full"}
                                ...
                                <th className={" table-cell p-0 border-none"} colSpan={1}>
                                    <a
                                        className={CTAButtonMixin + " w-full"}
                                        rel="noopener noreferrer"
                                        target="_blank"
...
function getEntryClass(value: React.ReactNode) {
    if (value === "Yes") return "bg-green-100 text-green-900";
    if (value === "Pro") return "bg-blue-100 text-blue-900 font-bold";
    if (value === "Plus" || value === "Enterprise") return "bg-blue-100 text-blue-900";
    if (value === "WIP") return "bg-yellow-100 text-yellow-900";
    return "";
}
