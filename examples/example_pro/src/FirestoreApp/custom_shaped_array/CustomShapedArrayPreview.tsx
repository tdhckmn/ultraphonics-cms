...
import { Property, PropertyPreview, PropertyPreviewProps } from "@firecms/core";
...
export default function CustomShapedArrayPreview({
    ...
    if (!customProps) throw Error("Properties not specified");
        ...
        <div>
            {properties.map((property, index) => (
                <div className="m-2" key={`custom_shaped_array_${index}`}>
                    <PropertyPreview property={property} value={value[index]} size={size} />
                </div>
            ))}
        </div>
    );
