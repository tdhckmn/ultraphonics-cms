...
export const algoliaSearchControllerBuilder = buildExternalSearchController({
    ...
    search: async ({ path, searchString }) => {
        ...
        if (path === "users") return performAlgoliaTextSearch(client, "users", searchString);
        if (path === "blog") return performAlgoliaTextSearch(client, "blog", searchString);
        if (path === "books") return performAlgoliaTextSearch(client, "books", searchString);
        return undefined;
    },
});
...
export const pineconeSearchControllerBuilder = buildPineconeSearchController({
    ...
    search: async ({ path, searchString, currentUser }) => {
        if (!currentUser) {
