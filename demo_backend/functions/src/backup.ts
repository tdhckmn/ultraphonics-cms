    ...
    await firestore
        ...
        .then((snapshot) =>
            snapshot.docs.forEach((d) => {
                if (!blogIds.includes(d.id)) d.ref.delete();
            })
        );
    await firestore
        .collection("/products")
        ...
        .then((snapshot) =>
            snapshot.docs.forEach((d) => {
                if (!productIds.includes(d.id)) d.ref.delete();
            })
        );
...
async function deleteCollection(collectionRef: admin.firestore.CollectionReference) {
    ...
    const deletionPromises = snapshot.docs.map(async (doc) => {
        ...
        const subcollectionDeletions = subcollections.map((subcoll) => deleteCollection(subcoll));
...
const blogIds = [
    "Ewsn3fq2tfLiIZg8uwzM",
    "OLRPBl5dA1JZhcSlhLkF",
    "aD11ISLz4zgxOei5q9Es",
    "c4o9TlWEDcwCJoifI6ra",
    "lB9HSU4wF4cSxdFC0Uro",
    "x20cgjhZwjYCA2XvhF9y",
];
