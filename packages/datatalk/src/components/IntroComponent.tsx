...
export function IntroComponent({
    ...
    const promptSuggestions =
        (dataTalk.rootPromptsSuggestions ?? []).length > 0
    ...
    return (
        <div className={"my-8"}>
            ...
            <Typography paragraph={true} className={"ml-4 my-2"}>
                DataTalk is a conversational interface to your data. You can ask questions, run
                commands and explore your data in a natural way.
            </Typography>
            {promptSuggestions && (
                <>
                    ...
                    <div className={"flex gap-1 sm:gap-2 md:gap-4 overflow-auto no-scrollbar"}>
                        {promptSuggestions.map((prompt, index) => (
                            <PromptSuggestion
                                key={index}
                                onClick={onPromptSuggestionClick}
                                prompt={prompt}
                            />
                        ))}
                    </div>
...
function PromptSuggestion({
    ...
    return (
        <Card
            className={
                "px-4 pt-12 pb-4 border-none w-[220px] min-w-[140px] font-semibold flex items-end"
            }
