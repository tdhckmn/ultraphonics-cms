    ...
    return (
        <>
                ...
                parsedElements.map((element, index) => {
                    if (element.type === "html") {
                        return (
                            <div
                                className={
                                    "max-w-full prose dark:prose-invert prose-headings:font-title text-base text-surface-700 dark:text-surface-200"
                                }
            ...
            <div className={"mt-2 flex flex-row gap-1"}>
                {canRegenerate && (
                    <Tooltip title={"Regenerate"} asChild={true}>
                ...
                <BadMessageIcon disabled={loading} onFeedback={onFeedback} />
            </div>
        </>
    );
}

function MessageCopyIcon({ text, disabled }: { text: string; disabled?: boolean }) {
...
function BadMessageIcon({
    ...
    return (
        <>
            <Tooltip title={dialogOpen ? undefined : "Bad response"} asChild={true}>
                <IconButton
                    size={"smallest"}
                    disabled={disabled}
                    onClick={() => {
...
function FeedbackLabel({
    ...
    return (
        <Label
            ...
            className={
                value === selected
                    ? "bg-surface-300 dark:bg-surface-700 hover:bg-surface-300 hover:dark:bg-surface-700"
                    : ""
            }
            onClick={() => {
                setSelected(value);
