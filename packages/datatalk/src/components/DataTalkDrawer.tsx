...
export function DataTalkDrawer() {
    const { drawerHovered, drawerOpen, openDrawer, closeDrawer } = useApp();
    ...
    return (
        <>
            <div className={"flex-grow overflow-scroll no-scrollbar my-8"}>
                {!drawerOpen && (
                    <div
                        className={"flex justify-center p-4 h-full cursor-pointer"}
                        onClick={openDrawer}
                    >
                        <ManageSearchIcon />
                    </div>
                ...
                {sessions?.map((session, index) => {
                    ...
                    const charsLimit = 30;
                    const firstChars =
                        firstMessage?.text.slice(0, charsLimit) ?? "DataTalk session started";
                    return (
                        <NavLink
                            key={index}
                            // onClick={onClick}
                            ...
                            className={({ isActive }: any) =>
                                cls(
                                    ...
                                    "font-medium text-sm",
                                    isActive
                                        ? "bg-surface-accent-200 bg-opacity-60 dark:bg-surface-800 dark:bg-opacity-30"
                                        : ""
                                )
                            }
