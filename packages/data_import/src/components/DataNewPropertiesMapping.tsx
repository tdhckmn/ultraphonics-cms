                ...
                <TableBody>
                    {destinationProperties &&
                        headingsOrder.map((importKey) => {
                            ...
                            const property = mappedKey
                                ? (getPropertyInPath(destinationProperties, mappedKey) as Property)
                                : null;

                            const originProperty = getPropertyInPath(originProperties, importKey) as
                                | Property
                                | undefined;
                            const originDataType = originProperty
                                ...
                                : undefined;
                            return (
                                <TableRow key={importKey} style={{ height: "90px" }}>
                                    ...
                                    <TableCell
                                        className={importKey === idColumn ? "text-center" : undefined}
                                        style={{ width: "75%" }}
                                    >
                                        {buildPropertyView?.({
                    ...
                    <TableBody>
                        {destinationProperties &&
                            getAllPropertyKeys(destinationProperties).map((key) => {
                                ...
                                return (
                                    <TableRow key={key} style={{ height: "70px" }}>
                                        <TableCell style={{ width: "20%" }}>
                                        ...
                                        <TableCell
                                            className={key === idColumn ? "text-center" : undefined}
                                            style={{ width: "75%" }}
                                        >
                                            <DefaultValuesField
                                                property={property}
    ...
    return (
        <div>
            <Select
                ...
                renderValue={(value) => {
                    return (
                        <Typography variant={"body2"}>{value !== "__none__" ? value : "Autogenerate ID"}</Typography>
                    );
                }}
