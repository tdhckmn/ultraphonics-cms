...
export function ImportSaveInProgress<C extends EntityCollection>({
    ...
    function save() {
        ...
        saveDataBatch(dataSource, collection, path, importConfig.entities, 0, 25, setProcessedEntities)
            .then(() => {
    ...
    if (errorSaving) {
        return (
            <CenteredView className={"flex flex-col gap-4 items-center"}>
                <Typography variant={"h6"}>Error saving data</Typography>
    ...
    return (
        <CenteredView className={"flex flex-col gap-4 items-center"}>
            ...
            <Typography variant={"caption"}>Do not close this tab or the import will be interrupted.</Typography>
        </CenteredView>
    );
}
