...
export function buildCollectionId(idOrPath: string, parentCollectionIds?: string[]): string {
    ...
    return [...parentCollectionIds.map(stripCollectionPath), stripCollectionPath(idOrPath)].join(
        COLLECTION_PATH_SEPARATOR
    );
}
...
export const docsToCollectionTree = (docs: DocumentSnapshot[]): PersistedCollection[] => {
    ...
    const orderedKeys = Object.keys(collectionsMap).sort(
        (a, b) =>
            b.split(COLLECTION_PATH_SEPARATOR).length - a.split(COLLECTION_PATH_SEPARATOR).length
    );
    ...
    orderedKeys.forEach((id) => {
        ...
        if (id.includes(COLLECTION_PATH_SEPARATOR)) {
            const parentId = id
                ...
                .slice(0, -1)
                .join(COLLECTION_PATH_SEPARATOR);
...
export function prepareCollectionForPersistence<
    M extends {
        [Key: string]: CMSType;
    },
>(collection: Partial<PersistedCollection<M>>, propertyConfigs: Record<string, PropertyConfig>) {
    ...
    const cleanedProperties = inputProperties
        ? cleanPropertyConfigs(inputProperties, propertyConfigs)
        : undefined;
    const properties = cleanedProperties
        ? setUndefinedToDelete(removeFunctions(cleanedProperties))
        : undefined;
    ...
    if (rest.propertiesOrder || properties) {
        newCollection.propertiesOrder = removeDuplicates(
            rest.propertiesOrder ?? Object.keys(properties)
        );
    }

    delete newCollection.permissions;
    if (newCollection.entityViews) {
        newCollection.entityViews = newCollection.entityViews.filter(
...
function cleanPropertyConfigs(
    ...
    Object.entries(properties).forEach(([key, property]) => {
        if (typeof property === "object") {
            const config = property.propertyConfig
