...
export function useFirestoreDelegate({
    ...
    useEffect(() => {
        if (!searchControllerRef.current && firebaseApp) {
            if (
                (textSearchControllerBuilder || localTextSearchEnabled) &&
    ...
    const buildQuery = useCallback(
        <M>(
            ...
            const collectionReference: Query = collectionGroup
                ? collectionGroupClause(firestore, path)
            ...
            if (filter) {
                Object.entries(filter)
                    ...
                    .forEach(([key, filterParameter]) => {
                        ...
                        queryParams.push(
                            whereClause(key, op, cmsToFirestoreModel(value, firestore))
                        );
                    });
            }
    ...
    const cmsToDelegateModel = useCallback(
        (values: any) => {
            if (!firebaseApp) throw Error("useFirestoreDelegate Firebase not initialised");
    ...
    return {
        ...
        initTextSearch: useCallback(
            async (props: {
                ...
                console.debug(
                    "Init text search controller",
                    searchControllerRef.current,
                    props.path
                );
                if (!searchControllerRef.current) {
                    console.warn(
                        "You are trying to use text search, but have not provided a text search controller in `useFirestoreDelegate`. You can also set the flag `localTextSearchEnabled` to use local search in `useFirestoreDelegate`. Local text search can incur in performance issues and higher costs for large datasets."
                    );
                    return false;
                }
                try {
                        ...
                        if (!searchString)
                            onUpdate(
                                snapshot.docs.map((doc) =>
                                    createEntityFromDocument(doc, databaseId)
                                )
                            );
                    },
                    error: onError,
        ...
        fetchEntity: useCallback(
            <M extends Record<string, any>>({
                ...
                getAndBuildEntity(path, entityId, collection?.databaseId),
            [getAndBuildEntity]
        ),
        ...
        deleteEntity: useCallback(
            <M extends Record<string, any>>({ entity }: DeleteEntityProps<M>): Promise<void> => {
                if (!firebaseApp) throw Error("useFirestoreDelegate Firebase not initialised");
        ...
        checkUniqueField: useCallback(
            async (
                ...
                const q = query(
                    collectionClause(firestore, path),
                    whereClause(name, "==", cmsToFirestoreModel(value, firestore))
                );
        ...
        isFilterCombinationValid: useCallback(
            ({
                ...
                const values: [WhereFilterOp, any][] = Object.values(filterValues) as [
                    WhereFilterOp,
                    any,
                ][];
                    ...
                    indexes
                        ...
                        .find((compositeIndex) =>
                            Object.entries(filterValues).every(
                                ([key, value]) =>
                                    compositeIndex[key] !== undefined &&
                                    (!sortDirection || compositeIndex[key] === sortDirection)
                            )
                        ) !== undefined
                );
            },
...
export function cmsToFirestoreModel(data: any, firestore: Firestore, inArray = false): any {
    if (data === undefined) {
        ...
        return data
            ...
            .map((v) => cmsToFirestoreModel(v, firestore, true));
        ...
        return Object.entries(data)
            .map(([key, v]) => {
                ...
                if (firestoreModel !== undefined) return { [key]: firestoreModel };
                else return {};
            })
            .reduce((a, b) => ({ ...a, ...b }), {});
    }
