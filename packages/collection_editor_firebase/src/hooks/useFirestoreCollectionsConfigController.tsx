...
export interface CollectionConfigControllerProps<
    EC extends PersistedCollection,
 ...
 * @param configPath
 */
export function useFirestoreCollectionsConfigController<
    EC extends PersistedCollection,
    USER extends User = User,
>({
    firebaseApp,
    ...
    const collectionsConfigPath =
        configPath ??
        (generalConfigPath
            ? removeInitialAndTrailingSlashes(generalConfigPath) + "/collections"
            : undefined) ??
    ...
    const [persistedCollections, setPersistedCollections] = React.useState<
        PersistedCollection[] | undefined
    >();
    ...
    const deleteCollection = useCallback(
        ({ id, parentCollectionIds }: DeleteCollectionParams): Promise<void> => {
            if (!firebaseApp || !collectionsConfigPath)
    ...
    const saveNavigationEntries = useCallback(
        (navigationEntries: NavigationGroupMapping[]): Promise<void> => {
            if (!firebaseApp)
                throw Error("useFirestoreConfigurationPersistence Firestore not initialised");
            if (!generalConfigPath) {
                console.warn(
                    "No generalConfigPath provided in useFirestoreConfigurationPersistence, cannot save homepage navigation entries order"
                );
            ...
            return runTransaction(firestore, async (transaction) => {
                transaction.set(
                    ref,
                    { homePage: { navigationEntries: navigationEntries } },
                    { merge: true }
                );
            });
        },
    ...
    const saveCollection = useCallback(
        <M extends Record<string, any>>({
            ...
            if (!firebaseApp || !collectionsConfigPath)
                throw Error("useFirestoreConfigurationPersistence Firestore not initialised");
            const firestore = getFirestore(firebaseApp);
            if (!id) throw Error("Trying to save a collection with no id");
            ...
            const cleanedCollection = prepareCollectionForPersistence(
                collectionData,
                propertyConfigsMap
            );
            const strippedPath = buildCollectionId(id, parentCollectionIds);
            const previousStrippedId = previousId
                ? buildCollectionId(previousId, parentCollectionIds)
                : undefined;
    ...
    const updateCollection = useCallback(
        <M extends Record<string, any>>({
            ...
            const strippedPath = buildCollectionId(id, parentCollectionIds);
            const previousStrippedPath = previousId
                ? buildCollectionId(previousId, parentCollectionIds)
                : undefined;
            const ref = doc(firestore, collectionsConfigPath, strippedPath);
    ...
    const collections =
        persistedCollections !== undefined
            ? applyPermissionsFunctionIfEmpty(
                  persistedCollections,
                  permissions as PermissionsBuilder<any, any>
              )
            : undefined;
    ...
    const saveProperty = useCallback(
        ({
            ...
            return runTransaction(firestore, async (transaction) => {
                const data = {
                    [namespaceToPropertiesPath(namespace) + "." + propertyKey]:
                        setUndefinedToDelete(removeFunctions(removeUndefined(property))),
                };
