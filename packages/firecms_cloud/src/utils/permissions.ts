...
export function resolveCollectionConfigPermissions<U extends User = User>({
    ...
    const userRoles: Role[] | undefined = fireCMSUser
        ? getUserRoles(userManagement.roles, fireCMSUser)
        : undefined;
    ...
    return userRoles
        .map((role) => ({
            ...
            editCollections:
                role.isAdmin ||
                role.config?.editCollections === true ||
                (role.config?.editCollections === "own" &&
                    collection?.ownerId === fireCMSUser?.uid),
            deleteCollections:
                role.isAdmin ||
                role.config?.deleteCollections === true ||
                (role.config?.deleteCollections === "own" &&
                    collection?.ownerId === fireCMSUser?.uid),
        }))
        .reduce(mergeConfigPermissions, baseConfigPermissions);
}

const mergeConfigPermissions = (
    permA: CollectionEditorPermissions,
    permB: CollectionEditorPermissions
) => {
    return {
