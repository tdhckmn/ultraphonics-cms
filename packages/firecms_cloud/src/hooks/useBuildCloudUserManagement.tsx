...
export function useBuildCloudUserManagement({
    ...
    useEffect(() => {
        ...
        return onSnapshot(
            query(collection(firestore, configPath, "users"), orderBy("created_on", "asc")),
            {
                next: (snapshot) => {
    ...
    const saveUser = useCallback(
        async (user: FireCMSCloudUserWithRoles): Promise<FireCMSCloudUserWithRoles> => {
            const firestore = firestoreRef.current;
    ...
    const saveRole = useCallback(
        <M extends { [Key: string]: CMSType }>(role: Role): Promise<void> => {
            const firestore = firestoreRef.current;
            if (!firestore || !configPath)
                throw Error("useFirestoreConfigurationPersistence Firestore not initialised");
    ...
    const removeUser = useCallback(
        async (user: FireCMSCloudUserWithRoles): Promise<void> => {
            const firestore = firestoreRef.current;
            if (!firestore || !configPath)
                throw Error("useFirestoreConfigurationPersistence Firestore not initialised");
    ...
    const loggedInUser = users.find(
        (u) => u.email.toLowerCase() === fireCMSBackend.user?.email?.toLowerCase()
    );
