...
export function useLicensesForUserController(): LicensesController {
    ...
    useEffect(() => {
        ...
        return onSnapshot(
            query(licensesRef, where("created_by", "==", userId), where("archived", "==", false)),
            {
                next: async (snapshot) => {
    ...
    const createLicense = async (license: Partial<ProLicense>): Promise<ProLicense> => {
        const firestore = firestoreRef.current;
        if (!firestore || !userId) throw new Error("Firestore not initialized");
        const licensesRef = collection(firestore, LICENSES_COLLECTION);
    ...
    const updateLicense = async (id: string, license: Partial<ProLicense>) => {
        ...
        return updateDoc(licensesRef, license).then(() => {
            return license as ProLicense;
        });
    };

    const listenSubscriptionsForLicense = useCallback(
        (
            licenseId: string,
            onSubscriptionsUpdate: (subs: Subscription[]) => void,
            onError: (error: Error) => void
            ...
            if (!firestore || !userId) throw new Error("Firestore not initialized");
            const subsRef = query(
                collection(firestore, CUSTOMERS_COLLECTION, userId, SUBSCRIPTIONS_COLLECTION),
                where("metadata.licenseId", "==", licenseId)
            );
