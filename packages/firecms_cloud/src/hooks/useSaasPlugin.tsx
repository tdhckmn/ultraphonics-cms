...
export function useSaasPlugin({
    ...
    const additionalChildrenStart = (
        <>
            {showHistoryAlert && (
                ...
                >
                    <>
                        🕒 You can now enable document history to keep track of your document
                        changes.
                        <Typography variant={"caption"}>
                            You can enable this feature for all your collections or just for
                            specific ones. Data will be stored in your Firestore database as a
                            sub-collection of each document.
                        </Typography>
                    </>
                </Alert>
            )}
    ...
    const additionalChildrenEnd = (
        <>
            <DataTalkSuggestions
                ...
                onAnalyticsEvent={onAnalyticsEvent}
            />
            <RootCollectionSuggestions introMode={introMode} onAnalyticsEvent={onAnalyticsEvent} />
        </>
    );
    ...
    return {
        ...
        collection: {
            injectCollections: projectConfig.isTrialOver
                ? undefined
                : useCallback(
        ...
        collectionView: {
            blockSearch: ({ context, path, collection, parentCollectionIds }) => {
...
export function IntroWidget({
    ...
    const hasCollections = navigation.initialised && (navigation.collections ?? []).length > 0;
    ...
    return (
        <div className={"mt-8 flex flex-col p-2"}>
            ...
            {hasCollections && introMode === "existing_project" && (
                <>
                    <Typography paragraph={true} className={"mt-4"}>
                        FireCMS Cloud is able to automatically set up collections for you, using the
                        data you have in your database and AI. This might take a few minutes, but it
            ...
            <Typography paragraph={true} className={"mt-4"}>
                FireCMS can be used as a standalone admin panel but it shines when you add your own
                custom functionality. Including your own custom components, fields, actions, views,
                and more.
            </Typography>
            <div className={"mb-8"}>
                ...
                <div
                    className={
                        "ml-2 select-all font-mono bg-surface-100 text-surface-800 dark:bg-surface-800 dark:text-white p-2 px-3  border-surface-200 border-solid w-fit text-md font-bold inline-flex rounded-md"
                    }
                >
