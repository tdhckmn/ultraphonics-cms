...
export function useBuildProjectConfig({
    ...
    const isTrialOver =
        (subscriptionPlan === "free" && trialValidUntil ? new Date() > trialValidUntil : false) ||
        false;
    ...
    const [serializedAppCheck, setSerializedAppCheck] = useState<SerializedAppCheckOptions | null>(
        null
    );
    ...
    useEffect(() => {
        if (primaryColor) {
            ...
            document.documentElement.style.setProperty(
                "--fcms-primary-bg",
                hexToRgbaWithOpacity(primaryColor, 10)
            );
        } else {
            document.documentElement.style.setProperty(
                "--fcms-primary",
                darkenColor(DEFAULT_PRIMARY_COLOR, 10)
            );
            document.documentElement.style.setProperty(
                "--fcms-primary-bg",
                hexToRgbaWithOpacity(DEFAULT_PRIMARY_COLOR, 10)
            );
        }
        if (secondaryColor) {
    ...
    const updateLocalTextSearchEnabled = useCallback(
    ...
    const updateHistoryDefaultEnabled = useCallback(
        async (enabled: boolean): Promise<void> => {
            ...
            return setDoc(
                doc(firestore, configPath),
                { history_default_enabled: enabled },
                { merge: true }
            );
        },
        [configPath]
        ...
        return onSnapshot(doc(firestore, projectPath), {
            next: (snapshot) => {
                ...
                if (updatedSerializedAppCheck) {
                    const appCheckOptions = updatedSerializedAppCheck
                        ? deserializeAppCheckOptions(updatedSerializedAppCheck)
                        : undefined;
    ...
    const updateAppCheck = useCallback(
        async (appCheck: SerializedAppCheckOptions | null): Promise<void> => {
            if (!backendFirebaseApp) throw Error("useBuildProjectConfig Firebase not initialised");
            const firestore = getFirestore(backendFirebaseApp);
            if (!firestore || !configPath)
                throw Error("useFirestoreConfigurationPersistence Firestore not initialised");
    ...
    return {
        ...
        customizationRevision:
            loadedProjectIdRef.current !== projectId ? undefined : customizationRevision,
        ...
        clientFirebaseConfig:
            loadedProjectIdRef.current !== projectId ? undefined : clientFirebaseConfig,
        clientFirebaseMissing:
            loadedProjectIdRef.current !== projectId ? undefined : clientFirebaseMissing,
        serviceAccountMissing:
            loadedProjectIdRef.current !== projectId ? undefined : serviceAccountMissing,
...
const uploadFile = (
    ...
    { file, fileName, path, metadata }: UploadFileProps
