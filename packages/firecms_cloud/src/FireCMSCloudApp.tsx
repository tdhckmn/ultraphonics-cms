...
export function FireCMSCloudApp({
    ...
    if (backendConfigLoading || !backendFirebaseApp) {
        ...
        component = <ErrorView error={backendFirebaseConfigError} />;
    } else if (configError) {
        component = <ErrorView error={configError} />;
    ...
    return <BrowserRouter basename={basePath}>{component}</BrowserRouter>;
}
...
function FullLoadingView(props: {
    ...
    return (
        <Scaffold key={"project_scaffold_" + props.projectId}>
...
export const FireCMSClient = function FireCMSClient({
    ...
    if (
        ...
        (!projectConfig.clientFirebaseConfig && !projectConfig.configError)
    ) {
        return (
            <FullLoadingView
                projectId={projectId}
...
function ErrorDelegatingLoginView({
    ...
    const errorBody =
        "code" in configError ? (
            <CloudErrorView error={configError} fireCMSBackend={fireCMSBackend} onFixed={onFixed} />
        ) : (
            <>
                <Typography>{configError.message}</Typography>
                <Typography>
                    This error may be caused when a user has been deleted from the client project.
                    Make sure a user exists in the client project with the same email as the one
                    trying to log in. If the problem persists, reach us at{" "}
                    <a
    ...
    return (
        <CenteredView maxWidth={"2xl"} className={"flex flex-col gap-4"}>
            ...
            <div>{errorBody}</div>
            <div>
                Some login errors may be caused by the user not being registered in the client
                project. Make sure a user exists in the client project with the same email as the
...
function NoAccessErrorView(props: { projectId: string; configError: Error; onLogout: () => void }) {
    return (
        <CenteredView maxWidth={"2xl"} className={"flex flex-col gap-4"}>
            ...
            <Typography>
                This error may be caused when trying to access with a user that is not registered in
                the project <code>{props.projectId}</code>. You can ask the project owner to add you
                to the project.
            </Typography>
...
export function FireCMSClientWithController({
    ...
    const fireCMSUser = useMemo(() => {
        ...
        if (!user) return;
        return userManagement.users.find(
            (fireCMSUser) => fireCMSUser.email.toLowerCase() === user?.email?.toLowerCase()
        );
    }, [
    ...
    const { delegatedLoginLoading, delegatedLoginError, checkLogin } = useDelegatedLogin({
        projectsApi: fireCMSBackend.projectsApi,
        firebaseApp: clientFirebaseApp,
        projectId,
        onUserChanged: (user) => {
    ...
    const permissions: PermissionsBuilder<PersistedCollection, FireCMSCloudUserWithRoles> =
        useCallback(({ collection, user }) => {
    ...
    // }
        ...
        loadingOrErrorComponent = (
            <ErrorDelegatingLoginView
                configError={delegatedLoginError}
                fireCMSBackend={fireCMSBackend}
        ...
        loadingOrErrorComponent = (
            <NoAccessError authController={authController} projectId={projectId} />
        );
    } else if (projectConfig.configError) {
        loadingOrErrorComponent = (
            <NoAccessErrorView
                configError={projectConfig.configError}
                projectId={projectId}
                onLogout={fireCMSBackend.signOut}
            />
        );
        ...
        loadingOrErrorComponent = (
            <CenteredView>
                ...
                <Typography variant={"body1"}>
                    This project has been blocked by the FireCMS team.
                </Typography>
                <Typography variant={"body1"}>
                    Please contact <a href={"mailto:hello@firecms.co"}>hello@firecms.co</a> for more
                    information
                </Typography>
            </CenteredView>
        );
    ...
    if (loadingOrErrorComponent) {
        return (
            <Scaffold key={"project_scaffold_" + projectConfig.projectId}>
                <AppBar logo={projectConfig?.logo ?? props.logo}>
                    {props.FireCMSAppBarComponent && (
                        <props.FireCMSAppBarComponent
...
function NoAccessError({
    ...
    if (
        ...
        (userManagement?.users ?? []).map((user) => user.email).includes(authController.user?.email)
    ) {
        error += " The user is registered in the project, but it is out of the free plan.";
    }
    return (
        <CenteredView maxWidth={"lg"} className={"gap-4"}>
            <ErrorView title={"You don't have access to the project " + projectId} error={error} />
            <Button variant="text" onClick={authController.signOut}>
                Sign out
            </Button>
        </CenteredView>
    );
...
function FireCMSAppAuthenticated({
    ...
    const adminRoutes = useMemo(
        ...
        [includeDataTalk, onAnalyticsEvent]
    );

    const configPermissions: CollectionEditorPermissionsBuilder<User, PersistedCollection> =
        useCallback(
            ({ user, collection }) =>
                            ...
                            {({ context, loading }) => {
                                ...
                                if (loading) {
                                    component = (
                                        ...
                                        >
                                            <AppBar logo={projectConfig.logo ?? logo}>
                                                {FireCMSAppBarComponent && (
                                                    <FireCMSAppBarComponent
                                                        title={projectConfig.projectName ?? ""}
                                                        {...appConfig?.fireCMSAppBarComponentProps}
...
function buildAdminRoutes(
    ...
    return views.map(({ path, name, view }) => (
        <Route key={"navigation_admin_" + path} path={path} element={view} />
    ));
}
