...
export function RootCollectionSuggestions({
    ...
    const filteredRootPathSuggestions = (rootPathSuggestions ?? []).filter(
        (path) => !existingPaths.includes(path)
    );

    useEffect(() => {
        const googleAccessToken = fireCMSBackend.googleCredential?.accessToken;
        fireCMSBackend.projectsApi
            .getRootCollections(projectConfig.projectId, googleAccessToken)
            .then((paths) => {
                setRootPathSuggestions(
                    paths.filter((p) => !existingPaths.includes(p.trim().toLowerCase()))
                );
            });
    }, []);
    ...
    return (
        <Collapse in={forceShowSuggestions || showSuggestions}>
            <div className={"flex flex-col gap-2 p-2 my-4"}>
                ...
                <div className={"flex flex-row gap-1 overflow-scroll no-scrollbar "}>
