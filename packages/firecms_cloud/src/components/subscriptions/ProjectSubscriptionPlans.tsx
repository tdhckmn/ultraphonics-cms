...
export function ProjectSubscriptionPlans() {
    const { subscriptionPlan, projectId, trialValidUntil } = useProjectConfig();

    if (!subscriptionPlan) throw new Error("No subscription plan");

    useBrowserTitleAndIcon("Plans");

    const { products, subscribeCloud, activeSubscriptions, getSubscriptionsForProject } =
    ...
    const cloudProducts = (products ?? []).filter(
        (p) => p.metadata?.type === "cloud_plus" || p.metadata?.type === "pro"
    );

    const plusProduct = cloudProducts.find((p) => p.metadata?.type === "cloud_plus");
    const plusSubscription = projectSubscriptions.find(
        (s) => s.product.metadata?.type === "cloud_plus"
    );
        ...
        <div className={"relative"}>
            ...
            <div className={cls("grid grid-cols-12 gap-4 items-center", loading ? "collapse" : "")}>
                <div className={"col-span-12 md:col-span-7 flex flex-col gap-2"}>
                    ...
                    {isSubscribed && (
                        <Typography variant={"subtitle1"} className="my-2">
                            ...
                            <Chip size={"small"}>FireCMS Cloud</Chip>.
                        </Typography>
                    )}
                    ...
                    {subscriptionPlan !== "cloud_plus" && plusProduct && (
                        <UpgradeCloudSubscriptionView
                            product={plusProduct}
                            projectId={projectId}
                ...
                <div className={cls("col-span-12 md:col-span-5")}>
                    <Card
                        className={
                            "p-6 bg-amber-200 dark:bg-amber-700 border-amber-300 dark:border-amber-800 flex flex-col gap-4"
                        }
                    >
                        <div>
                            These are some of the features you are already enjoying by using FireCMS
                            Cloud
                        </div>
...
function CurrentCloudSubscriptionView({ subscription }: CurrentSubscriptionViewProps) {
    ...
    return (
        <div key={subscription.id} className="mt-2 mb-2">
            <div>
                <>
                    The subscription is{" "}
                    <Chip
                ...
                <>
                    ...
                    <Chip size={"small"}>{getPriceString(subscription.price)}</Chip>per user.
                </>
