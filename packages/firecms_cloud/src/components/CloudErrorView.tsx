...
export function CloudErrorView({
    ...
    const { code, message, projectId } = error;

    if (code === "service-account-missing" && projectId && fireCMSBackend) {
        return (
            <CloudMissingServiceAccountErrorView
                projectId={projectId}
                fireCMSBackend={fireCMSBackend}
                message={
                    "Service account missing for this project. You need to link your project to a service account to use this feature."
                }
                onFixed={onFixed}
            />
        );
        ...
        return (
            <CloudMissingServiceAccountErrorView
                ...
                fireCMSBackend={fireCMSBackend}
                message={
                    "The service account linked to this project is invalid. You need to link your project to a valid service account to use this feature. The user fixing the issue must have the appropriate permissions."
                }
                onFixed={onFixed}
            />
        );
    } else if (code === "user-has-to-accept-googles-terms-of-service" && projectId) {
        return <CloudNeedsToAcceptTermsErrorView projectId={projectId} onFixed={onFixed} />;
    } else if (code === "user-has-no-previous-firebase-projects" && projectId) {
        return <CloudNoPreviousFirebaseProjectsErrorView projectId={projectId} onFixed={onFixed} />;
        ...
        return (
            <div className="flex flex-col space-y-2 py-4">
                ...
                <Typography>
                    Make sure the user exists in the client project and try again. If the problem
        ...
        return (
            <ServiceAccountMissingPermissions missingPermissions={error.data?.missingPermissions} />
        );
    }

    return (
        <div className="flex flex-col space-y-2 py-4">
            <Typography className="text-red-300">{message}</Typography>

            {error.data && (
                <pre className="text-xs text-gray-500 p-4">
...
function CloudMissingServiceAccountErrorView({
    ...
    const doCreateServiceAccount = async () => {
        ...
        return projectsApi
            .createServiceAccount(fireCMSBackend.googleCredential.accessToken, projectId, true)
    ...
    return (
        <div className="flex flex-col space-y-2 py-4">
            <Typography color={"error"}>
                You need to accept Google&apos;s terms of service before you can use this service.
            </Typography>
            ...
            <Button variant="outlined" color="error" onClick={onFixed} startIcon={<BuildIcon />}>
...
function CloudNoPreviousFirebaseProjectsErrorView({
    ...
    return (
        <div className="flex flex-col space-y-2 py-4">
            <Typography color={"error"}>
                You need to accept Firebase&apos;s terms of service before you can use this service.
            </Typography>
...
function ServiceAccountMissingPermissions(props: { missingPermissions: string[] | undefined }) {
    return (
        <div className="flex flex-col space-y-2 py-4">
            <Typography color={"error"}>Missing permissions</Typography>
            ...
            {props.missingPermissions && (
                <>
                    ...
                    <Typography variant={"caption"} component={"ul"}>
                        {props.missingPermissions.map((permission) => (
                            <li key={permission}>
