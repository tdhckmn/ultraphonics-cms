...
export function UsersTable({ onUserClicked }: { onUserClicked: (user: User) => void }) {
    const { users, saveUser, deleteUser } = useUserManagement<PersistedUser>();
    ...
    const dateUtilsLocale = customizationController?.locale
        ? locales[customizationController?.locale as keyof typeof locales]
        : undefined;
                ...
                <TableBody>
                    {users &&
                        users.map((user) => {
                            ...
                            const formattedDate = user.created_on
                                ? format(user.created_on, dateFormat, { locale: dateUtilsLocale })
                                : "";
                            ...
                            return (
                                ...
                                >
                                    <TableCell className={"w-10"}>
                                        <Tooltip asChild={true} title={"Delete this user"}>
                                            <IconButton
                                                size={"small"}
                                                onClick={(event) => {
                                    ...
                                    <Button
                                        ...
                                        onClick={() => {
                                            if (!authController.user?.uid) {
                                                throw Error(
                                                    "UsersTable, authController misconfiguration"
                                                );
                                            }
                                            saveUser({
