...
export function AdvancedTextField<T extends string | number>({
    ...
    if (disabled) {
        return <TextField label={label} disabled={true} value={internalValue} />;
    }
        ...
        >
            ...
            <TextareaAutosize
                className={cls(
                    ...
                    disabled &&
                        "border border-transparent outline-none opacity-50 text-surface-600 dark:text-surface-500",
...
function addLineBreaks(value?: string | number, addLastBreak = false) {
    ...
    if (lines.length === 1) return <span className="break-words">{value ?? " "}</span>;
    // @ts-ignore
