...
export function FormEnhanceAction({
    ...
    const { suggestions, getSamplePrompts } = dataEnhancementController;

    const loadingPrompts = useRef(false);
    const updateSuggestedPrompts = useCallback(
        async function updateSuggestedPrompts(instructions?: string) {
            ...
            setSamplePrompts(
                [...recentPromptsFromStorage, ...prompts.filter((p) => !recentPrompts.includes(p.prompt))].slice(0, 5),
            );
    ...
    const enhance = (prompt?: string) => {
        ...
        return dataEnhancementController
            .enhance({
                entityId,
    ...
    return (
        ...
        >
                        ...
                        {samplePrompt.type === "recent" && (
                            <IconButton
                                onClick={(e) => {
                                    ...
                                    setSamplePrompts(
                                        (samplePrompts ?? []).filter((p) => p.prompt !== samplePrompt.prompt),
                                    );
                                }}
                                size={"smallest"}
                            >
            ...
            >
                <TextareaAutosize
                    className={cls(
                        "p-4 rounded-lg resize-none bg-surface-100 dark:bg-surface-800 mx-2 w-full flex-grow outline-none",
                        focusedDisabled,
                    )}
                    value={instructions}
                ...
                >
                    ...
                    {!loading && <SendIcon color={"primary"} />}
                </IconButton>
            </div>
        </Menu>
    );
...
function getPromptsForExistingEntities(properties: PropertiesOrBuilders): SamplePrompt[] {
    ...
    const multilinePrompt: Property | undefined =
        multilineProperties.length > 0
...
const removeRecentPrompt = (storageKey: string, prompt: string) => {
    localStorage.setItem(
        storageKey,
        JSON.stringify(
            getRecentPromptsFromStorage(storageKey)
                .map((e) => e.prompt)
                .filter((e) => e !== prompt),
        ),
    );
