...
export function fieldBuilder<T extends CMSType = CMSType>(
    params: PluginFieldBuilderParams<T>,
): React.ComponentType<FieldProps<T>> | null {
    const { fieldConfigId, property } = params;

    const wrappedComponent = React.useMemo(
        () =>
            function FieldWrapper(props: FieldProps<T, any, any>) {
                const { enabled, suggestions, enhance, loadingSuggestions, editorAIController } =
                ...
                const filledCharacters = countStringCharacters(
                    props.context.values,
                    props.context.collection?.properties ?? {},
                );
...
const FieldInner = React.memo(
    function FieldInner<T extends CMSType = CMSType, M extends Record<string, any> = any>({
        loading,
        props,
        suggestedValue,
        enabled,
        ...
        const showEnhanceIcon =
            !props.disabled &&
            (!props.value ||
                (property.dataType === "string" && (property.multiline || property.markdown)) ||
                (property.dataType === "array" && property.of?.dataType === "string"));

        const indexOfSuggestion =
            props.value &&
            typeof props.value === "string" &&
            typeof suggestedValue === "string" &&
            props.value.endsWith(suggestedValue)
                ? props.value.indexOf(suggestedValue) + 1
                : undefined;

        const highlightRange =
            indexOfSuggestion && typeof suggestedValue === "string"
                ? {
        ...
        if (property.dataType === "string" && property.markdown) {
            ...
            fieldBinding = (
                <EnhanceTextFieldBinding {...(props as FieldProps<any>)} highlight={suggestedValue as string} />
            );
        } else {
        ...
        return (
            <>
                ...
                {showEnhanceIcon && (
                    <div
                        className={cls(
                            "dark:bg-surface-700 bg-surface-100 rounded-full absolute",
                            rightClass,
                            topClass,
                        )}
                    >
                                ...
                                {allowInstructions && (
                                    <div className={"p-4"}>
                                        <TextField
                                            ...
                                            size={"small"}
                                            className={
                                                "w-[400px] max-w-full text-text-primary dark:text-text-primary-dark"
                                            }
