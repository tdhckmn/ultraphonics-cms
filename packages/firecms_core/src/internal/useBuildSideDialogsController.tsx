...
export function useBuildSideDialogsController(): SideDialogsController {
    ...
    useEffect(() => {
        ...
        if (
            !equal(
                sidePanelsRef.current.map((p) => p.key),
                newPanels.map((p) => p.key)
            )
        ) {
    ...
    const close = useCallback(() => {
        ...
        if (routesCount.current > 0) {
            if (lastSidePanel.urlPath)
                // if it has a url path, we need to navigate back, don't remove this code
                navigate(-1);
            routesCount.current--;
    ...
    const open = useCallback(
        (panelProps: SideDialogPanelProps | SideDialogPanelProps[]) => {
            const newPanels: SideDialogPanelProps[] = Array.isArray(panelProps)
                ? panelProps
                : [panelProps];
