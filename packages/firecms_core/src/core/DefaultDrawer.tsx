...
export function DefaultDrawer({
    ...
    const { drawerHovered, drawerOpen, closeDrawer, logo } = useApp();
    ...
    const buildGroupHeader = useCallback(
        (group?: string) => {
            ...
            return (
                <div className="pl-6 pr-8 py-4 flex flex-row items-center">
                    <Typography
                        variant={"caption"}
                        color={"secondary"}
    ...
    return (
        <>
            <div
                className={cls("flex flex-col h-full relative flex-grow w-full", className)}
                style={style}
            >
                ...
                >
                    {groupsWithoutAdmin.map((group) => (
                        <div
                            className={
                                "bg-surface-50 dark:bg-surface-800 dark:bg-opacity-30 my-4 rounded-lg ml-3 mr-1"
                            }
 ...
 */
export function DrawerLogo({ logo }: { logo?: string }) {
    const navigation = useNavigationController();
    ...
    return (
        ...
        >
            <Tooltip title={"Home"} sideOffset={20} side="right">
                <Link className={"block"} to={navigation.basePath}>
                    {logo ? (
                        <img
                            src={logo}
                            alt="Logo"
                            className={cls(
                                "max-w-full max-h-full transition-all object-contain",
