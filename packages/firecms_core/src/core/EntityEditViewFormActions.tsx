...
export function EntityEditViewFormActions({
    ...
    const formActions = showDefaultActions
        ? entityActions.filter((a) => a.includeInForm === undefined || a.includeInForm)
        : [];
...
function buildBottomActions<M extends object>({
    ...
    return (
        <DialogActions position={"absolute"}>
            ...
            {entity && formActions.length > 0 && (
                <div className="flex-grow flex overflow-auto no-scrollbar">
...
function buildSideActions<M extends object>({
    ...
    return (
        <div
            className={cls(
                "overflow-auto h-full flex flex-col gap-2 w-80 2xl:w-96 px-4 py-16 sticky top-0 border-l",
                defaultBorderMixin
            )}
        >
            <LoadingButton
