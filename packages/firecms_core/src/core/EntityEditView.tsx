...
export function EntityEditView<M extends Record<string, any>, USER extends User>({
    ...
    const canEdit = useMemo(() => {
        if (status === "new" || status === "copy") {
            ...
            return entity
                ? canEditEntity(props.collection, authController, props.path, entity ?? null)
                : undefined;
        }
    }, [authController, entity, status]);

    if (
        (dataLoading && !cachedValues) ||
        ((!entity || canEdit === undefined) && (status === "existing" || status === "copy"))
    ) {
...
export function EntityEditViewInner<M extends Record<string, any>>({
    ...
    const defaultSelectedView = useMemo(
        () =>
            resolveDefaultSelectedView(collection ? collection.defaultSelectedView : undefined, {
                status,
                entityId,
            }),
        []
    ...
    const includeJsonView =
        collection.includeJsonView === undefined ? true : collection.includeJsonView;
    const hasAdditionalViews = customViewsCount > 0 || subcollectionsCount > 0 || includeJsonView;

    const plugins = customizationController.plugins;

    const { resolvedEntityViews, selectedEntityView, selectedSecondaryForm } =
        resolvedSelectedEntityView(customViews, customizationController, selectedTab, canEdit);

    const actionsAtTheBottom =
        !largeLayout || layout === "side_panel" || selectedEntityView?.includeActions === "bottom";

    const mainViewVisible = selectedTab === MAIN_TAB_VALUE || Boolean(selectedSecondaryForm);

    const authController = useAuthController();

    const customViewsView: React.ReactNode[] | undefined =
        customViews &&
        resolvedEntityViews
                        ...
                        <ErrorBoundary>
                            {usedFormContext && (
                                <Builder
                                    ...
                                    entity={usedEntity}
                                    modifiedValues={
                                        usedFormContext?.formex?.values ?? usedEntity?.values
                                    }
    ...
    const jsonView = (
        ...
        >
            <ErrorBoundary>
                <EntityJsonPreview values={formContext?.values ?? entity?.values ?? {}} />
            </ErrorBoundary>
        </div>
    );

    const subCollectionsViews =
        subcollections &&
        subcollections
            .map((subcollection) => {
                const subcollectionId = subcollection.id ?? subcollection.path;
                const newFullPath = usedEntity
                    ? `${path}/${usedEntity?.id}/${removeInitialAndTrailingSlashes(subcollection.path)}`
                    : undefined;
                const newFullIdPath = fullIdPath
                    ? `${fullIdPath}/${usedEntity?.id}/${removeInitialAndTrailingSlashes(subcollectionId)}`
                    : undefined;
                            ...
                            (usedEntity && newFullPath ? (
                                ...
                                <div className="flex items-center justify-center w-full h-full p-3">
                                    <Typography variant={"label"}>
                                        You need to save your entity before adding additional
                                        collections
                                    </Typography>
                                </div>
                            ))}
    ...
    const entityReadOnlyView =
        !canEdit && entity ? (
            ...
            >
                <div
                    className={cls(
                        "relative flex flex-col max-w-4xl lg:max-w-3xl xl:max-w-4xl 2xl:max-w-6xl w-full h-fit"
                    )}
                >
    ...
    const customViewTabsStart = resolvedEntityViews
        .filter((view) => view.position === "start")
        .map((view) => (
            <Tab
    ...
    const customViewTabsEnd = resolvedEntityViews
        .filter((view) => !view.position || view.position === "end")
        .map((view) => (
            <Tab
                className={!view.tabComponent ? "text-sm min-w-[120px]" : undefined}
                value={view.key}
    ...
    let result = (
        <div className="relative flex flex-col h-full w-full bg-white dark:bg-surface-900">
            {shouldShowTopBar && (
                <div
                    className={cls(
                        "h-14 flex overflow-visible overflow-x-scroll w-full no-scrollbar h-14 border-b pl-2 pr-2 pt-1 flex items-end bg-surface-50 dark:bg-surface-900",
                        defaultBorderMixin
                    )}
                >
                    {barActions}
