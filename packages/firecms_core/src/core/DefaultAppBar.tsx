...
export const DefaultAppBar = function DefaultAppBar({
    ...
    if (user) {
        const initial = user?.displayName
            ...
            : user?.email
              ...
              : "A";
        avatarComponent = <Avatar src={user.photoURL ?? undefined}>{initial}</Avatar>;
    ...
    return (
        ...
        >
                ...
                <div className="mr-8 hidden lg:block">
                    <div className={"flex flex-row gap-2"}>
                        {breadcrumbs.breadcrumbs.map((breadcrumb, index) => {
                            return (
                                <React.Fragment key={breadcrumb.url + "_" + index}>
            ...
            {endAdornment && <ErrorBoundary>{endAdornment}</ErrorBoundary>}
            ...
            {includeModeToggle && (
                ...
                >
                    <MenuItem onClick={() => setMode("dark")}>
                        <DarkModeIcon size={"smallest"} /> Dark
                    </MenuItem>
                    <MenuItem onClick={() => setMode("light")}>
                        <LightModeIcon size={"smallest"} /> Light{" "}
                    </MenuItem>
                    <MenuItem onClick={() => setMode("system")}>
                        {" "}
                        <BrightnessMediumIcon size={"smallest"} />
                        System
                    </MenuItem>
                </Menu>
            )}
