...
export function EntityPreview({
    ...
    const titleProperty = includeTitle
        ...
        : undefined;
    const imagePropertyKey = includeImage
        ? getEntityImagePreviewPropertyKey(resolvedCollection)
        : undefined;
    const imageProperty = imagePropertyKey
        ? resolvedCollection.properties[imagePropertyKey]
        : undefined;
    const usedImageProperty =
        imageProperty && "of" in imageProperty ? imageProperty.of : imageProperty;
    const restProperties = listProperties.filter(
        (p) => p !== titleProperty && p !== imagePropertyKey
    );

    const imageValue = imagePropertyKey
        ? getValueInPath(entity.values, imagePropertyKey)
        : undefined;
    ...
    return (
        ...
        >
            <div
                className={cls(
                    "flex w-10 h-10 ml-1 mr-2 shrink-0",
                    size === "small" ? "my-0.5" : "m-2 self-start"
                )}
            >
...
export const EntityPreviewContainer = React.forwardRef<HTMLDivElement, EntityPreviewContainerProps>(
    ({ children, hover, onClick, size, style, className, fullwidth = true, ...props }, ref) => {
        return (
            <div
                ref={ref}
                style={{
