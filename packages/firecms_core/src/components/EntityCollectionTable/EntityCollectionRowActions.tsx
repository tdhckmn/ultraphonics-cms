    ...
    return (
        ...
        >
            {(hasActions || selectionEnabled) && (
                <div className="w-34 flex justify-center">
                    {uncollapsedActions.map((action, index) => (
                        <Tooltip key={index} title={action.name} asChild={true}>
                            <IconButton
                                onClick={(event: MouseEvent) => {
                                    ...
                                    action.onClick({
                                        ...
                                        onCollectionChange,
                                        openEntityMode:
                                            openEntityMode ?? collection?.openEntityMode,
                                    });
                                }}
                    ...
                    {hasCollapsedActions && (
                        <Menu
                            trigger={
                                <IconButton size={largeLayout ? "medium" : "small"}>
                                    <MoreVertIcon />
                                </IconButton>
                            }
                        >
            ...
            {!hideId && size !== "xs" && (
                ...
                >
                    {hasDraft && (
                        <Tooltip title={"Local unsaved changes"} className={"inline"}>
