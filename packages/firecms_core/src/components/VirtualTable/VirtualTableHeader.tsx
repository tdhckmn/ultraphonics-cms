...
export const VirtualTableHeader = React.memo<VirtualTableHeaderProps<any>>(
    function VirtualTableHeader<M extends Record<string, any>>({
        ...
        const update = useCallback(
            (
                ...
                if (newOpenFilterState !== undefined) setOpenFilter(newOpenFilterState);
            },
            ...
            <ErrorBoundary>
                ...
                >
                    <div className="overflow-hidden flex-grow">
                        <div className={`flex items-center justify-${column.headerAlign} flex-row`}>
                            {column.icon}
                    ...
                    <>
                        ...
                        {column.sortable && (sort || hovered || openFilter) && (
                            <Badge color="secondary" invisible={!sort}>
                                <IconButton
                                    size={"small"}
                                    className={
                                        onHover || openFilter
                    ...
                    {column.filter && createFilterField && (
                        <div>
                            <Badge color="secondary" invisible={!filter}>
                                <Popover
                                    open={openFilter}
    ...
    equal
) as React.FunctionComponent<VirtualTableHeaderProps<any>>;
...
function FilterForm<M>({
    ...
    const [filterInternal, setFilterInternal] = useState<
        [VirtualTableWhereFilterOp, any] | undefined
    >(filter);
    ...
    return (
        ...
        >
            <div className={cls(defaultBorderMixin, "py-4 px-6 typography-label border-b")}>
                {column.title ?? id}
            </div>
            {filterField && <div className="m-4">{filterField}</div>}
