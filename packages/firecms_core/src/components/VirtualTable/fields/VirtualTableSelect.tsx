...
export function VirtualTableSelect(props: {
    ...
    const validValue =
        (Array.isArray(internalValue) && multiple) || (!Array.isArray(internalValue) && !multiple);
    ...
    const onChange = useCallback(
        (updatedValue: string | string[]) => {
            ...
            if (valueType === "number") {
                ...
                if (!updatedValue) updateValue(null);
    ...
    return multiple ? (
        <MultiSelect
            ...
            includeClear={false}
            useChips={false}
            value={validValue ? (internalValue as any[]).map((v) => v.toString()) : []}
            onValueChange={onChange}
        >
            {enumValues?.map((enumConfig) => (
                <MultiSelectItem key={enumConfig.id} value={String(enumConfig.id)}>
                    <EnumValuesChip
