...
export function StringNumberFilterField({
    ...
    function updateFilter(
        ...
        if (typeof newValue === "number" && isNaN(newValue)) newValue = undefined;
            ...
            <div className="flex-grow ml-2 flex flex-col gap-2">
                {!enumValues && (
                    <TextField
                        ...
                        endAdornment={
                            internalValue && (
                                <IconButton onClick={(e) => updateFilter(operation, undefined)}>
                                    <CloseIcon />
                                </IconButton>
                            )
                        }
                ...
                {enumValues && !multiple && (
                    <Select
                        ...
                        renderValue={(enumKey) => {
                            if (enumKey === null) return "Filter for null values";
                    ...
                    >
                        {enumValues.map((enumConfig) => (
                            <SelectItem
                                key={`select_item_${name}_${enumConfig.id}`}
                ...
                {enumValues && multiple && (
                    <MultiSelect
                        position={"item-aligned"}
                        value={
                            Array.isArray(internalValue) ? internalValue.map((e) => String(e)) : []
                        }
                        onValueChange={(value) => {
                            updateFilter(
                                operation,
                                dataType === "number" ? value.map((v) => parseInt(v)) : value
                            );
                        }}
                        multiple={multiple}
                        endAdornment={
                    ...
                    >
                        {enumValues.map((enumConfig) => (
                            <MultiSelectItem
                                key={`select_value_${name}_${enumConfig.id}`}
                                value={String(enumConfig.id)}
                            >
                                <EnumValuesChip
                                    enumKey={String(enumConfig.id)}
