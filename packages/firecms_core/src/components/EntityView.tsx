...
export function EntityView<M extends Record<string, any>>({
    ...
    const resolvedCollection: ResolvedEntityCollection<M> = useMemo(
        () =>
            resolveCollection<M>({
                collection,
                path,
        ...
        <div className={"w-full " + className}>
            <div className={"w-full mb-4"}>
                <div
                    className={cls(
                        defaultBorderMixin,
                        "flex justify-between py-2 border-b last:border-b-0"
                    )}
                >
                    <div className="flex items-center w-1/4">
                        <span className="pl-2 text-sm text-surface-600">Id</span>
                    </div>
                    <div className="flex-grow p-2 ml-2 w-3/4 text-surface-900 dark:text-white min-h-[56px] flex items-center">
                ...
                {Object.entries(properties).map(([key, property]) => {
                    ...
                    return (
                        ...
                        >
                            <div className="flex items-center w-1/4">
                                <span className="pl-2 text-sm text-surface-600">
                                    {property.name}
                                </span>
                            </div>
                            <div className="flex-grow p-2 ml-2 w-3/4 text-surface-900 dark:text-white min-h-[56px] flex items-center">
