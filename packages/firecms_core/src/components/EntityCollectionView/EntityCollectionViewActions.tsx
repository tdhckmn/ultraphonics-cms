...
export function EntityCollectionViewActions<M extends Record<string, any>>({
    ...
    const addButton =
        ...
        (largeLayout ? (
            ...
            <Button id={`add_entity_${path}`} onClick={onNewClick} variant="filled" color="primary">
                <AddIcon />
            </Button>
        ));
            ...
            <Tooltip
                title={
                    multipleDeleteEnabled
                        ? "Delete"
                        : "You have selected at least one entity you cannot delete"
                }
            >
                {button}
            </Tooltip>
    ...
    const actions = toArray(collection.Actions).map((Action, i) => (
    ...
    if (plugins) {
        plugins.forEach((plugin, i) => {
            if (plugin.collectionView?.CollectionActions) {
                actions.push(
                    ...toArray(plugin.collectionView?.CollectionActions).map((Action, j) => (
                        <ErrorBoundary key={`plugin_actions_${i}_${j}`}>
                            <Action
                                {...actionProps}
                                {...plugin.collectionView?.collectionActionsProps}
                            />
                        </ErrorBoundary>
                    ))
                );
    ...
    return (
        <>
            <ErrorBoundary>{actions}</ErrorBoundary>
            {multipleDeleteButton}
            {addButton}
        </>
    );
