...
export function NavigationGroup({
    ...
    return (
        ...
        >
            <div
                className={`flex items-center ${isPreview ? "px-1 py-0.5 m-0" : "ml-3.5 mt-6"} `}
            ...
            {isPreview ? (
                ...
                <div
                    className={cls(
                        "mt-4 p-8 bg-surface-accent-200 dark:bg-surface-accent-800 rounded-lg"
                    )}
                ...
                <div className={cls("mt-4", !minimised ? "pt-0" : "")}>{children}</div>
            )}
        </div>
    );
}
