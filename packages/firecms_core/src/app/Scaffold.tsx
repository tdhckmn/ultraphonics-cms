 ...
 * @group Core
 */
export const Scaffold = React.memo<PropsWithChildren<ScaffoldProps>>(function Scaffold(
  props: PropsWithChildren<ScaffoldProps>
) {
  const { children, autoOpenDrawer, logo, className, style, padding = true } = props;

  const drawerChildren = React.Children.toArray(children).filter(
    (child: any) => child.type.componentType === 'Drawer'
  );
  if (drawerChildren.length > 1) {
    throw Error('Only one Drawer component is allowed in Scaffold');
  }
  const appBarChildren = React.Children.toArray(children).filter(
    (child: any) => child.type.componentType === 'AppBar'
  );
  if (appBarChildren.length > 1) {
    throw Error('Only one AppBar component is allowed in Scaffold');
  }
  const otherChildren = React.Children.toArray(children).filter(
    (child: any) => child.type.componentType !== 'Drawer' && child.type.componentType !== 'AppBar'
  );
  ...
  const computedDrawerOpen: boolean =
    drawerOpen || Boolean(largeLayout && autoOpenDrawer && onHover);
  ...
  return (
    ...
    >
      <div
        className={cls(
          'flex h-screen w-screen bg-surface-50 dark:bg-surface-900 text-surface-900 dark:text-white overflow-hidden',
          className
        )}
        style={{
      ...
      >
        ...
        <main className="flex flex-col flex-grow overflow-auto">
          ...
          >
            <ErrorBoundary>{otherChildren}</ErrorBoundary>
          </div>
        </main>
      </div>
    </AppContext.Provider>
...
const DrawerHeader = () => {
  return <div className="flex flex-col min-h-16"></div>;
};
  ...
  const innerDrawer = (
    <div
      ...
      style={{
        width,
        transition:
          'left 100ms cubic-bezier(0.4, 0, 0.6, 1) 0ms, opacity 100ms cubic-bezier(0.4, 0, 0.6, 1) 0ms, width 100ms cubic-bezier(0.4, 0, 0.6, 1) 0ms',
      }}
    >
      {!props.open && props.displayed && (
        <Tooltip title="Open menu" side="right" sideOffset={12} asChild={true}>
          <div className="ml-2 fixed top-1 left-2 sm:top-2 sm:left-2 !bg-surface-50 dark:!bg-surface-900 rounded-full w-fit z-20">
            <IconButton
              color="inherit"
      ...
      >
        <IconButton aria-label="Close drawer" onClick={() => props.setDrawerOpen(false)}>
          <ChevronLeftIcon />
        </IconButton>
      </div>
