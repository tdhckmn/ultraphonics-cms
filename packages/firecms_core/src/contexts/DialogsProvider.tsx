...
export const DialogsControllerContext = React.createContext<DialogsController>(
...
export const DialogsProvider: React.FC<PropsWithChildren<{}>> = ({ children }) => {
    ...
    const open = useCallback(
        <T extends object = object>(dialogEntry: DialogControllerEntryProps<T>) => {
            const updatedPanels = [...dialogEntriesRef.current, dialogEntry];
            updateDialogEntries(updatedPanels);

            return {
                closeDialog: () => {
                    const updatedPanels = dialogEntriesRef.current.filter(
                        (e) => e.key !== dialogEntry.key
                    );
                    updateDialogEntries(updatedPanels);
                },
            };
