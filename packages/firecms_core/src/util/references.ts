...
export function getEntityPreviewKeys(
    ...
    if (!listProperties && targetCollection.previewProperties) {
        listProperties = targetCollection.previewProperties?.filter((p) =>
            allProperties.includes(p as string)
        );
    }
    if (listProperties && listProperties.length > 0) {
        ...
        return listProperties
            .filter((key) => {
                ...
                return (
                    propertyOrBuilder &&
                    !isPropertyBuilder(propertyOrBuilder) &&
                    !isReferenceProperty(authController, propertyOrBuilder, fields)
                );
            })
            .slice(0, limit);
    }
...
export function getEntityTitlePropertyKey<M extends Record<string, any>>(
...
export function getEntityImagePreviewPropertyKey<M extends object>(
    ...
    for (const key in collection.properties) {
        ...
        if (
            ...
            property.storage?.acceptedFiles?.includes("image/*")
        ) {
            return key;
        }
    }
    ...
    for (const key in collection.properties) {
        ...
        if (
            ...
            property.of.storage?.acceptedFiles?.includes("image/*")
        ) {
            return key;
        }
    }
