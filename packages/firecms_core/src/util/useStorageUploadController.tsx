...
export function useStorageUploadController<M extends object>({
    ...
    const storage: StorageConfig | undefined =
        property.dataType === "string"
            ? property.storage
            : property.dataType === "array" && (property.of as Property).dataType === "string"
    ...
    const internalInitialValue: StorageFieldItem[] = getInternalInitialValue(
    ...
    const fileNameBuilder = useCallback(
        async (file: File) => {
            if (storage.fileName) {
    ...
    const storagePathBuilder = useCallback(
        (file: File) => {
            return (
    ...
    const onFileUploadComplete = useCallback(
        async (uploadedPath: string, entry: StorageFieldItem, metadata?: any) => {
        ...
        async (acceptedFiles: File[]) => {
            ...
            if (processFile) {
                try {
                    ...
                    console.error(
                        "Error processing file with custom code. Attempting to continue uploading.",
                        e
                    );
                }
            }
...
function getInternalInitialValue(
...
function removeDuplicates(items: StorageFieldItem[]) {
    return items.filter((item, i) => {
        return (
            (items.map((v) => v.storagePathOrDownloadUrl).indexOf(item.storagePathOrDownloadUrl) ===
                i ||
                !item.storagePathOrDownloadUrl) &&
...
const compressionFormat = (file: File) =>
    supportedTypes[file.type] ? supportedTypes[file.type] : null;

const defaultQuality = 100;
const resizeAndCompressImage = (file: File, compression: ImageCompression) =>
    new Promise<File>((resolve) => {
        const inputQuality =
            compression.quality === undefined ? defaultQuality : compression.quality;
