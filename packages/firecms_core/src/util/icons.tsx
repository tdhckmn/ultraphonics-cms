...
export function getIcon(
    iconKey?: string | React.ReactNode,
    className?: string,
    color?: IconColor,
    ...
    if (typeof iconKey === "string") {
        ...
        return usedIconKey in iconKeysMap ? (
            <Icon iconKey={usedIconKey} size={size} className={className} color={color} />
        ) : undefined;
    }
...
export const IconForView = React.memo(
    function IconForView({
        ...
        if (collectionOrView?.icon && icon) return icon;

        let slugName = slugify(
            ("singularName" in collectionOrView ? collectionOrView.singularName : undefined) ??
                collectionOrView.name
        );

        let key: string | undefined;
        if (slugName in iconKeysMap) key = slugName;
        ...
        if (!key) key = coolIconKeys[hashString(collectionOrView.path) % iconsCount];

        return <Icon iconKey={key} size={size} className={className} color={color} />;
    },
    (prevProps, nextProps) => {
        return (
            equal(prevProps.collectionOrView?.icon, nextProps.collectionOrView?.icon) &&
