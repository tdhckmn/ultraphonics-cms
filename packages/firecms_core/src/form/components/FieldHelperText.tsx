...
export function FieldHelperText({
    ...
    if (!(showError && error) && (!includeDescription || !hasDescription)) return null;
    ...
    return (
        <div className={"flex ml-3.5 mt-1"}>
            ...
            {property.longDescription && (
                <Tooltip title={property.longDescription} side="bottom" asChild={true}>
                    <IconButton size={"small"} className="self-start">
                        <InfoIcon color={"disabled"} size={"small"} />
                    </IconButton>
                </Tooltip>
            )}
        </div>
