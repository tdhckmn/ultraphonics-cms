...
export function KeyValueFieldBinding({
    ...
    if (!property.keyValue) {
        throw Error(
            `Your property ${propertyKey} needs to have the 'keyValue' prop in order to use this field binding`
        );
    }
...
function MapEditView<T extends Record<string, any>>({
    ...
    const updateDataType = (rowId: number, dataType: DataType) => {
        ...
        setValue({
            ...(value ?? ({} as T)),
            [internalState.find((row) => row[0] === rowId)?.[1].key ?? ""]:
    ...
    return (
        <div className="py-1 flex flex-col gap-1">
            {internalState.map(([rowId, { key: fieldKey, dataType }], index) => {
                const entryValue = fieldKey ? value?.[fieldKey] : "";
                const onFieldKeyChange = (newKey: string) => {
                    setInternalState(
                        internalState.map((currentRowId) => {
                ...
                return (
                    <MapKeyValueRow
                        ...
                        onDeleteClick={() => {
                            ...
                            setInternalState(
                                internalState.filter((currentRowId) => currentRowId[0] !== rowId)
                            );
                            setValue({
                                ...newValue,
                            });
...
function MapKeyValueRow<T extends Record<string, any>>({
    ...
    function buildInput(entryValue: any, fieldKey: string, dataType: DataType) {
        if (dataType === "string" || dataType === "number") {
            ...
            return (
                <div className={cls(defaultBorderMixin, "ml-2 pl-2 border-l border-solid")}>
                    <ArrayContainer
                        value={entryValue}
                        newDefaultEntry={""}
        ...
        <>
            ...
            >
                ...
                <div className="flex-grow">
                    {dataType !== "map" &&
                        dataType !== "array" &&
                        buildInput(entryValue, fieldKey, dataType)}
                </div>
                <div className={"flex flex-col"}>
                    ...
                    >
                        <MenuItem dense onClick={() => doUpdateDataType("string")}>
                            string
                        </MenuItem>
                        <MenuItem dense onClick={() => doUpdateDataType("number")}>
                            number
                        </MenuItem>
                        <MenuItem dense onClick={() => doUpdateDataType("boolean")}>
                            boolean
                        </MenuItem>
                        <MenuItem dense onClick={() => doUpdateDataType("date")}>
                            date
                        </MenuItem>
                        <MenuItem dense onClick={() => doUpdateDataType("map")}>
                            map
                        </MenuItem>
                        <MenuItem dense onClick={() => doUpdateDataType("array")}>
                            array
                        </MenuItem>
                    </Menu>

                    <IconButton aria-label="delete" size={"smallest"} onClick={onDeleteClick}>
                        <RemoveIcon size={"smallest"} />
                    </IconButton>
                </div>
            </Typography>

            {(dataType === "map" || dataType === "array") &&
                buildInput(entryValue, fieldKey, dataType)}
        </>
    );
}
...
function ArrayKeyValueRow<T>({
    ...
    function buildInput(entryValue: any, dataType: DataType) {
        if (dataType === "string" || dataType === "number") {
            ...
            return <Typography variant={"caption"}>Arrays of arrays are not supported.</Typography>;
        } else if (dataType === "map") {
            return (
                <div className={cls(defaultBorderMixin, "ml-2 pl-2 border-l border-solid")}>
                    <MapEditView
    ...
    return (
        <>
            ...
            >
                ...
                <Menu
                    trigger={
                        <IconButton size={"small"} className="h-7 w-7">
                            <ArrowDropDownIcon />
                        </IconButton>
                    }
                >
