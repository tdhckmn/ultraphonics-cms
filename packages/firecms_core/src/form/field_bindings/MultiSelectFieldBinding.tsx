...
export function MultiSelectFieldBinding({
    ...
    if (!enumValues) {
        ...
        throw Error(
            "Field misconfiguration: array field of type string or number needs to have enumValues"
        );
    }
    ...
    const renderValue = useCallback(
        (enumKey: string, list: boolean) => {
            const enumValue =
                enumKey !== undefined ? getLabelOrConfigFrom(enumValues, enumKey) : undefined;
            return (
                <EnumValuesChip
                    enumKey={enumKey}
    ...
    return (
        <>
                ...
                onValueChange={(updatedValue: string[]) => {
                    ...
                    if (of && (of as ResolvedProperty)?.dataType === "number") {
                        newValue = updatedValue
                            ? (updatedValue as string[]).map((e) => parseFloat(e))
                            : [];
                    } else {
                        newValue = updatedValue;
                    }
            ...
            >
                {enumValues
                    .map((enumValue) => String(enumValue.id))
                    .map((enumKey) => (
