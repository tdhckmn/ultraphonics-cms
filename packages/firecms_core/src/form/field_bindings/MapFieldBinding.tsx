...
export function MapFieldBinding({
    ...
    if (pickOnlySomeKeys) {
        if (value) {
            mapProperties = pick(
                property.properties,
                ...Object.keys(value).filter((key) => key in property.properties!)
            );
        } else {
            mapProperties = {};
        }
    ...
    const mapFormView = (
        <>
            <div
                className={cls(
                    "py-1 flex flex-col space-y-2",
                    minimalistView && property.widthPercentage !== undefined ? "mt-8" : undefined
                )}
            >
                {Object.entries(mapProperties)
                    ...
                    .map(([entryKey, childProperty], index) => {
                        const thisDisabled =
                            isReadOnly(childProperty) || Boolean(childProperty.disabled);
                        ...
                        return (
                            <div key={`map-${propertyKey}-${index}`} className={"relative"}>
                                <ErrorBoundary>
                                    <PropertyFieldBinding {...fieldBindingProps} />
                                </ErrorBoundary>
                            </div>
                        );
                    })}
        ...
        <ErrorBoundary>
            {!minimalistView && (
                <ExpandablePanel
                    ...
                    innerClassName={
                        "px-2 md:px-4 pb-2 md:pb-4 pt-1 md:pt-2 bg-white dark:bg-surface-900"
                    }
                    title={
            ...
            <FieldHelperText
                ...
                error={
                    error && !partOfArray
                        ? typeof error === "string"
                            ? error
                            : "A property of this map has an error"
                        : undefined
                }
                disabled={disabled}
                property={property}
...
const buildPickKeysSelect = (
    disabled: boolean,
    properties: Properties,
    setValue: (value: any) => void,
    value: any
) => {
    const keys = Object.keys(properties).filter((key) => !value || !(key in value));
