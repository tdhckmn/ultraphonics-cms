...
export const ReferencePreview = function ReferencePreview(props: ReferencePreviewProps) {
    ...
    if (
        !(
            ...
            "isEntityReference" in reference &&
            reference.isEntityReference()
        )
    ) {
        console.warn("Reference preview received value of type", typeof reference);
        return (
            <EntityPreviewContainer onClick={props.onClick} size={props.size}>
                <ErrorView
                    error={"Unexpected value. Click to edit"}
...
function ReferencePreviewInternal({
    ...
    if (!collection) {
        if (customizationController.components?.missingReference) {
            ...
            throw Error(
                `Couldn't find the corresponding collection view for the path: ${reference.path}`
            );
        }
    }
...
function ReferencePreviewExisting<M extends Record<string, any> = any>({
    ...
    if (!reference) {
        ...
        body = <ErrorView error={"Reference does not exist"} tooltip={reference.path} />;
    }
    if (body) {
        return (
            <EntityPreviewContainer
