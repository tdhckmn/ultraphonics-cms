...
export function ImagePreview({ size, url }: ImagePreviewProps) {
    ...
    return (
        ...
        >
            <img src={url} className={"rounded-md"} style={imageStyle} />

            <div
                className={
                    "flex flex-row gap-2 absolute bottom-[-4px] right-[-4px] invisible group-hover:visible"
                }
            >
                {navigator && (
                    <Tooltip asChild={true} title="Copy url to clipboard" side={"bottom"}>
                        <IconButton
                            variant={"filled"}
                            size={"smallest"}
                            onClick={(e) => {
