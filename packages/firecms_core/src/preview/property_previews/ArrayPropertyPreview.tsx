...
export function ArrayPropertyPreview({
    ...
    if (!property.of) {
        throw Error(
            `You need to specify an 'of' prop (or specify a custom field) in your array property ${propertyKey}`
        );
    }
        ...
        <div className="flex flex-col gap-2">
            {values &&
                values.map((value, index) => {
                    const of: ResolvedProperty =
                        property.resolvedProperties[index] ??
                        property.resolvedProperties[index] ??
                        (Array.isArray(property.of) ? property.of[index] : property.of);
                    return of ? (
                        <React.Fragment key={"preview_array_" + index}>
                            <div
