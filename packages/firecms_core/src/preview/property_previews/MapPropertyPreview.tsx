...
export function MapPropertyPreview<T extends Record<string, any> = Record<string, any>>({
    ...
    if (!mapProperty.properties || Object.keys(mapProperty.properties ?? {}).length === 0) {
        return <KeyValuePreview value={value} />;
    }
        ...
        return (
            <div className="w-full flex flex-col space-y-1 md:space-y-2">
                {mapPropertyKeys.map((key, index) => (
                    <div key={`map_${key}`}>
                        <ErrorBoundary key={"map_preview_" + mapProperty.name + key + index}>
                            <PropertyPreview
                                propertyKey={key}
                                value={value[key]}
            ...
            {mapPropertyKeys &&
                mapPropertyKeys.map((key, index) => {
                    ...
                    return (
                        ...
                        >
                            <div className={"flex flex-row pt-0.5 pb-0.5 gap-2"}>
                                <div className="min-w-[140px] w-[25%] py-1">
                                    <Typography
                                        variant={"caption"}
                    ...
                    <div className={"flex flex-row pt-0.5 pb-0.5 gap-2"}>
                        ...
                        <div className="flex-grow max-w-[75%]">
                            {childValue && typeof childValue !== "object" && (
                                <Typography>
                                    <ErrorBoundary>{childValue.toString()}</ErrorBoundary>
                                </Typography>
                            )}
                        </div>
                    </div>
