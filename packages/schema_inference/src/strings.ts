...
export function findCommonInitialStringInPath(valuesCount?: ValuesCountEntry) {
    ...
    function getPath(value: any) {
        ...
        console.warn(
            "findCommonInitialStringInPath: value is not a string or document with path",
            value
        );
        return undefined;
    }

    const strings: string[] = valuesCount.values
        .map((v) => getPath(v))
        .filter((v) => !!v) as string[];
    const pathWithSlash = strings.find((s) => s.includes("/"));
    if (!pathWithSlash) return undefined;

    const searchedPath = pathWithSlash.substr(0, pathWithSlash.lastIndexOf("/"));

    const yep =
        valuesCount.values.filter((value) => {
            const path = getPath(value);
            if (!path) return false;
            return path.startsWith(searchedPath);
        }).length >
        (valuesCount.values.length / 3) * 2;
