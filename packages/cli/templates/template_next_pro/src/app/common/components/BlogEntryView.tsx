        ...
        <div>
            ...
            <Container className={"mb-16"}>
                <Container maxWidth={"5xl"} className={"my-12"}>
                    {blogEntry?.name && (
                        <Typography variant={"h1"} className="mt-16 mb-8 mx-4 md:mx-12">
                            {blogEntry.name}
                        </Typography>
                    )}
                    {blogEntry?.tags && (
                ...
                {blogEntry?.content &&
                    blogEntry.content
                        ...
                        .map((entry: any, index: number) => {
                            ...
                            if (entry.type === "quote")
                                return (
                                    <Quote key={`preview_text_${index}`} quoteText={entry.value} />
                                );
                            if (entry.type === "images")
                                return (
                                    <Images key={`preview_images_${index}`} images={entry.value} />
                                );
...
export function Images({ images }: { images: string[] }) {
    ...
    return (
        <div className="flex justify-center my-1 sm:my-3 md:my-6">
            {images.map((url, index) => (
                <div key={`images_${index}`} className=" w-full h-[350px]">
...
function ProductGroupPreview({ products }: { products: ProductWithId[] }) {
    ...
    return (
        <div
            className={cls("flex gap-4 items-center max-w-5xl mx-auto px-4 md:px-12", {
...
function Quote({ quoteText }: { quoteText: string }) {
    ...
    return (
        <Container
            maxWidth={"5xl"}
            className={"border-l-2 border-l-red-950 dark:border-l-red-100 my-8 mx-8 italic"}
        >
