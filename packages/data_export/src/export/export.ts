...
export function getEntityCSVExportableData(
    data: Entity<any>[],
    additionalData: Record<string, any>[] | undefined,
...
export function getEntityJsonExportableData(
    data: Entity<any>[],
    additionalData: Record<string, any>[] | undefined,
    properties: ResolvedProperties,
    dateExportType: "timestamp" | "string",
...
function getExportHeaders<M extends Record<string, any>>(
    properties: ResolvedProperties<M>,
    propertiesOrder: string[] | undefined,
    additionalHeaders: string[] | undefined,
    arrayValuesCount?: ArrayValuesCount,
): Header[] {
    const headers: Header[] = [
        ...
        ...(propertiesOrder ?? Object.keys(properties)).flatMap((childKey) => {
            ...
            if (arrayValuesCount && arrayValuesCount[childKey] > 1) {
                return Array.from({ length: arrayValuesCount[childKey] }, (_, i) =>
                    getHeaders(property as ResolvedProperty, `${childKey}[${i}]`, ""),
                ).flat();
            } else {
...
function processValueForExport(
    inputValue: any,
            ...
            if (Array.isArray(property.of)) {
                ...
                value =
                    exportType === "csv"
                        ...
                        : inputValue.map((e) =>
                              processValueForExport(e, property.of as ResolvedProperty, exportType, dateExportType),
                          );
            } else {
...
function processValuesForExport<M extends Record<string, any>>(
    ...
    const updatedValues = Object.entries(properties)
        .map(([key, property]) => {
            ...
            const updatedValue = processValueForExport(
                inputValue,
                property as ResolvedProperty,
                exportType,
                dateExportType,
            );
