...
export function ExportCollectionAction<M extends Record<string, any>, USER extends User>({
    ...
    const collection: ResolvedEntityCollection<M> = React.useMemo(
        () =>
            resolveCollection({
                collection: inputCollection,
                path,
            ...
            const resolvedExportColumnsValues: Record<string, any>[] = additionalExportFields
                ? await Promise.all(
                      entities.map(async (entity) => {
                          return (
                              await Promise.all(
                                  additionalExportFields.map(async (column) => {
                                      return {
    ...
    const doDownload = useCallback(
        async (collection: ResolvedEntityCollection<M>, exportConfig: ExportConfig<any> | undefined) => {
    ...
    return (
        <>
            <Tooltip title={"Export"} asChild={true}>
                <IconButton color={"primary"} onClick={handleClickOpen}>
                    <DownloadIcon />
                </IconButton>
            </Tooltip>

            <Dialog open={open} onOpenChange={setOpen} maxWidth={"xl"}>
                ...
                <DialogContent className={"flex flex-col gap-4 my-4"}>
                    ...
                    {collectionEntitiesCount > DOCS_LIMIT && (
                        <Alert color={"warning"}>
                            <div>This collections has a large number of documents ({collectionEntitiesCount}).</div>
                        </Alert>
                    )}
                        ...
                        <div className={"p-4 flex flex-col"}>
                            <div className="flex items-center">
                                <input
                                    ...
                                    className={cls(
                                        "w-4 bg-surface-100 border-surface-300 dark:bg-surface-700 dark:border-surface-600",
                                    )}
                                />
                                <label
                                    htmlFor="radio-csv"
                                    className="p-2 text-sm font-medium text-surface-900 dark:text-surface-accent-300"
                                >
                                    CSV
                                </label>
                            </div>
                        ...
                        <div className={"p-4 flex flex-col"}>
                            <div className="flex items-center">
                                ...
                                <label
                                    ...
                                    className="p-2 text-sm font-medium text-surface-900 dark:text-surface-accent-300"
                                >
                                    Dates as timestamps ({dateRef.current.getTime()})
                                </label>
                            </div>
                            <div className="flex items-center">
                                <input
                                ...
                                <label
                                    ...
                                    className="p-2 text-sm font-medium text-surface-900 dark:text-surface-accent-300"
                                >
                                    Dates as strings ({dateRef.current.toISOString()})
                                </label>
                            </div>
                        </div>
                    </div>
                ...
                <DialogActions>
                    ...
                    <Button variant="filled" onClick={onOkClicked} disabled={dataLoading || !canExport}>
                        Download
                    </Button>
                </DialogActions>
            </Dialog>
