...
export function EditorCollectionActionStart({
    ...
    if (
        !equal(getObjectOrNull(tableController.filterValues), getObjectOrNull(collection.initialFilter)) ||
        ...
        saveDefaultFilterButton = (
            <>
                <Tooltip
                    asChild={true}
                    title={
                        tableController.sortBy || tableController.filterValues
                            ? "Save default filter and sort"
                            : "Clear default filter and sort"
                    }
                >
                    <Button
                        color={"primary"}
                ...
                {(collection.initialFilter || collection.initialSort) && (
                    <Tooltip title={"Reset to default filter and sort"}>
                        <Button
                            color={"primary"}
                            size={"small"}
                            variant={"text"}
                            onClick={() => {
                                ...
                                if (collection?.initialSort) tableController.setSortBy?.(collection?.initialSort);
                            }}
                        >
                            <UndoIcon />
                        </Button>
    ...
    return <>{canEditCollection && saveDefaultFilterButton}</>;
}
