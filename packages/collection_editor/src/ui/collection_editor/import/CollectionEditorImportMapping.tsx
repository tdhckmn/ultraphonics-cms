...
export function CollectionEditorImportMapping({
    ...
    const { setFieldValue, setFieldTouched, values } = useFormex<PersistedCollection>();
    ...
    const onPropertyChanged = ({ id, property, previousId, namespace }: OnPropertyChangedParams) => {
        ...
        const getCurrentPropertiesOrder = (namespace?: string) => {
            ...
            return (
                currentPropertiesOrderRef.current[namespace] ?? getIn(values, namespaceToPropertiesOrderPath(namespace))
            );
        };
                    ...
                    buildPropertyView={({ property, propertyKey, importKey, isIdColumn }) => {
                        ...
                        return (
                            <ImportNewPropertyFieldPreview
                                ...
                                onPropertyNameChanged={(propertyKey: string, value: string) =>
                                    setFieldValue(`properties.${propertyKey}.name`, value, false)
                                }
                                onEditClick={() => {
...
function PropertySelect({
    ...
    return (
        <Tooltip
            title={property && widget ? `${widget?.name} - ${property.dataType}` : undefined}
...
export function PropertySelectItem({ value, optionDisabled, propertyConfig, existing }: PropertySelectItemProps) {
    return (
        <SelectItem value={value} disabled={optionDisabled} className={"flex flex-row items-center"}>
            <div className={cls("flex flex-row items-center text-base min-h-[48px]", optionDisabled ? "w-full" : "")}>
                <div className={"mr-8"}>
                    <PropertyConfigBadge propertyConfig={propertyConfig} />
                </div>
                <div>
                    <div>{propertyConfig.name}</div>
                    <Typography variant={"caption"} color={"disabled"} className={"max-w-sm"}>
                        {existing && optionDisabled
                            ? "You can only switch to widgets that use the same data type"
