...
function EnumFormFields({
    ...
    const buildEntry = ({ index, internalId }: ArrayEntryParams) => {
    ...
    return (
        <div className={"col-span-12"}>
            <div className="ml-3.5 flex flex-row items-center">
                <ListIcon />
                <Typography variant={"subtitle2"} className="ml-2 grow">
                    Values
                </Typography>
                {allowDataInference && (
                    <Button
...
const EnumEntry = React.memo(
    function EnumEntryInternal({
        ...
        const { values, handleChange, errors, setFieldValue, touched } = useFormex<EnumValues>();
        ...
        return (
            <>
                <div className={"flex w-full align-center justify-center"}>
                    ...
                    {!disabled && (
                        <Badge color={"error"} invisible={!entryError?.id}>
                            <IconButton size="small" aria-label="edit" className={"m-1"} onClick={() => onDialogOpen()}>
                ...
                {entryError?.label && (
                    <Typography variant={"caption"} className={"ml-3.5 text-red-500 dark:text-red-500"}>
                        {entryError?.label}
                    </Typography>
                )}
...
function EnumEntryDialog({
    ...
    const { errors } = useFormex<EnumValues>();
    ...
    return (
        ...
        >
            ...
            <DialogActions>
                <Button autoFocus variant="outlined" onClick={onClose} color="primary">
                    Ok
                </Button>
            </DialogActions>
        </Dialog>
