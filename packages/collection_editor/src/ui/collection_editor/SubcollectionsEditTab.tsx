...
export function SubcollectionsEditTab({
    ...
    const resolvedEntityViews =
        (values.entityViews
            ?.filter((e) => typeof e === "string")
            .map((e) => resolveEntityView(e, contextEntityViews))
            .filter(Boolean) as EntityCustomView[]) ?? [];
    const hardCodedEntityViews =
        (collection.entityViews?.filter((e) => typeof e !== "string") as EntityCustomView[]) ?? [];
            ...
            <Container maxWidth={"2xl"} className={"flex flex-col gap-4 p-8 m-auto"}>
                <div className={"flex  flex-col gap-16"}>
                    <div className={"flex-grow flex flex-col gap-4 items-start"}>
                        <Typography variant={"h5"}>Subcollections of {values.name}</Typography>
            ...
            {viewToDelete && (
                <ConfirmationDialog
                    ...
                    onAccept={() => {
                        setFieldValue(
                            "entityViews",
                            values.entityViews?.filter((e) => e !== viewToDelete),
                        );
                    ...
                    body={
                        <>
                            This will <b>not delete any data</b>, only the view in the CMS
                        </>
                    }
                />
            )}
