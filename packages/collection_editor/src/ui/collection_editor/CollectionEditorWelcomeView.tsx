...
export function CollectionEditorWelcomeView({
            ...
            pathSuggestions(path)
                .then((suggestions) => {
                    const filteredSuggestions = suggestions.filter(
                        (s) =>
                            !(existingCollectionPaths ?? []).find(
                                (c) => c.trim().toLowerCase() === s.trim().toLowerCase(),
                            ),
                    );
    ...
    const { values, setFieldValue, setValues, submitCount } = useFormex<EntityCollection>();
    ...
    return (
        <div className={"overflow-auto my-auto"}>
            <Container maxWidth={"4xl"} className={"flex flex-col gap-4 p-8 m-auto"}>
                <div className="flex flex-row py-2 pt-3 items-center">
                ...
                <div className={"my-2"}>
                    ...
                    <div className={"flex gap-4"}>
                        <TemplateButton
                            ...
                            icon={<Icon size={"small"} iconKey={productsCollectionTemplate.icon! as string} />}
                            onClick={() => {
...
export function TemplateButton({
    ...
    return (
        <Tooltip title={subtitle} asChild={true}>
            <Card
                onClick={onClick}
                className={cls(
