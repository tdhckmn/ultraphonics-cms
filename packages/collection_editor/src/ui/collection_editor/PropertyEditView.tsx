...
export const PropertyForm = React.memo(
    function PropertyForm(props: PropertyFormProps) {
        ...
        const lastSubmittedProperty = useRef<OnPropertyChangedParams | undefined>(
            property
                ? {
                      id: propertyKey,
        ...
        const doOnPropertyChanged = ({ id, property }: OnPropertyChangedParams) => {
            const params = {
                id,
        ...
        const formexController = useCreateFormex<PropertyWithId>({
            ...
            initialValues: property ? ({ id: propertyKey, ...property } as PropertyWithId) : initialValue,
            initialErrors,
        ...
        return (
            <Formex value={formexController}>
                <PropertyEditFormFields
                    onPropertyChanged={onPropertyChangedImmediate ? doOnPropertyChanged : undefined}
                    onDelete={onDelete}
...
export function PropertyFormDialog({
    ...
    return (
        <Dialog open={open ?? false} maxWidth={"xl"} fullWidth={true}>
            <form
                noValidate={true}
                autoComplete={"off"}
                onSubmit={(e) => {
...
function PropertyEditFormFields({
    ...
    const [selectedFieldConfigId, setSelectedFieldConfigId] = useState<string | undefined>(
        values?.dataType ? getFieldId(values) : undefined,
    );
    ...
    let childComponent;
    if (
        selectedFieldConfigId === "text_field" ||
        selectedFieldConfigId === "multiline" ||
        selectedFieldConfigId === "email"
    ) {
        childComponent = (
            <StringPropertyField widgetId={selectedFieldConfigId} disabled={disabled} showErrors={showErrors} />
        );
    } else if (selectedFieldConfigId === "url") {
        childComponent = <UrlPropertyField disabled={disabled} showErrors={showErrors} />;
    } else if (selectedFieldConfigId === "markdown") {
        childComponent = <MarkdownPropertyField disabled={disabled} showErrors={showErrors} />;
    ...
    } else if (selectedFieldConfigId === "multi_select" || selectedFieldConfigId === "multi_number_select") {
        childComponent = (
            <EnumPropertyField
                multiselect={true}
                updateIds={!existing}
        ...
        childComponent = <StoragePropertyField existing={existing} multiple={false} disabled={disabled} />;
    } else if (selectedFieldConfigId === "multi_file_upload") {
        childComponent = <StoragePropertyField existing={existing} multiple={true} disabled={disabled} />;
        ...
        childComponent = (
            <ReferencePropertyField showErrors={showErrors} existing={existing} multiple={false} disabled={disabled} />
        );
        ...
        childComponent = (
            <ReferencePropertyField showErrors={showErrors} existing={existing} multiple={true} disabled={disabled} />
        );
        ...
        childComponent = <KeyValuePropertyField disabled={disabled} />;
    } else {
        childComponent = null;
    }
        ...
        <>
            {disabled && (
                <InfoLabel mode={"warn"}>
                    ...
                    <Typography variant={"caption"}>
                        You may not have permission to edit it or it is defined in code with no <code>editable</code>{" "}
                        flag
                    </Typography>
                </InfoLabel>
            )}
            ...
            <div className="flex mt-2 justify-between">
                <div className={"w-full flex flex-col gap-2"}>
                    ...
                    {selectedWidgetError && (
                        <Typography variant="caption" className={"ml-3.5"} color={"error"}>
                            Required
                        </Typography>
                    )}
            ...
            {onDelete && (
                <ConfirmationDialog
                    ...
                    body={
                        <div>
                            {" "}
                            This will <b>not delete any data</b>, only modify the collection.
                        </div>
                    }
                />
            )}
...
function WidgetSelectView({
    ...
    const displayedWidgets = (
        inArray
            ? allSupportedFields.filter(
                  ([_, propertyConfig]) =>
                      !isPropertyBuilder(propertyConfig.property) && propertyConfig.property?.dataType !== "array",
              )
            : allSupportedFields
    )
    ...
    const baseFieldConfig =
        baseProperty && !isPropertyBuilder(baseProperty) ? getFieldConfig(baseProperty, propertyConfigs) : undefined;
    const computedFieldConfig =
        baseFieldConfig && propertyConfig ? mergeDeep(baseFieldConfig, propertyConfig) : propertyConfig;

    const groups: string[] = [
        ...new Set(
            Object.keys(displayedWidgets).map((key) => {
    ...
    return (
        <>
            ...
            <Dialog open={open} onOpenChange={(open) => onOpenChange(open, Boolean(value))} maxWidth={"4xl"}>
                <DialogTitle>Select a property widget</DialogTitle>
                <DialogContent>
                    <div>
                        {groups.map((group) => {
                            return (
...
export function WidgetSelectViewItem({
    ...
    const baseProperty = propertyConfig.property;
    const shouldWarnChangingDataType =
        existing && !isPropertyBuilder(baseProperty) && baseProperty.dataType !== initialProperty?.dataType;

    return (
        <Card onClick={onClick} className={"flex flex-row items-center px-4 py-2 m-1"}>
            <div className={cls("flex flex-row items-center text-base min-h-[48px]")}>
                ...
                <div>
                    <div className={"flex flex-row gap-2 items-center"}>
                        {shouldWarnChangingDataType && (
                            <Tooltip
                                title={
                                    "This widget uses a different data type than the initially selected widget. This can cause errors with existing data."
                                }
                            >
                                <WarningIcon size="smallest" className={"w-4"} />
                            </Tooltip>
                        )}
                        <Typography color={shouldWarnChangingDataType ? "secondary" : undefined}>
                            {propertyConfig.name}
                        </Typography>
                    </div>

                    <Typography variant={"caption"} color={"secondary"} className={"max-w-sm"}>
