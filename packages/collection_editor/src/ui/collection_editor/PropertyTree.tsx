...
export const PropertyTree = React.memo(function PropertyTree<
    M extends {
        [Key: string]: CMSType;
    },
>({
...
export function PropertyTreeEntry({
    ...
    const editable =
        propertyOrBuilder &&
        ((collectionEditable && !isPropertyBuilder(propertyOrBuilder)) || editableProperty(propertyOrBuilder));

    return (
        <div ref={setNodeRef} style={style} className="relative -ml-8">
            <div className="relative">
                ...
                <div className="absolute top-2 right-2 flex flex-row">
                    {isPropertyInferred && (
                        <Tooltip title={"Inferred property"}>
                            <AutorenewIcon size="small" className={"p-2"} />
                        </Tooltip>
                    )}

                    {onPropertyRemove && !isPropertyInferred && (
                        <Tooltip title={"Remove"} asChild={true}>
                            <IconButton
                                size="small"
                                color="inherit"
                    ...
                    {onPropertyMove && (
                        <Tooltip title={"Move"} asChild={true}>
                            <IconButton component={"span"} size="small" {...attributes} {...listeners}>
