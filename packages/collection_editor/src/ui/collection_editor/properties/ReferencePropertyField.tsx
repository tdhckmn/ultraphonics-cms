...
export function ReferencePropertyField({
    ...
    const { values, handleChange, errors } = useFormex<StringProperty | NumberProperty>();
        ...
        <>
            <Select
                ...
                renderValue={(selected) => {
                    const selectedCollection = collections.find(
                        (collection) => collection.id === selected || collection.path === selected,
                    );
                    if (!selectedCollection) return null;
                    return (
                        <div className="flex flex-row">
                            <IconForView collectionOrView={selectedCollection} />
                            <Typography variant={"subtitle2"} className="font-medium ml-4">
            ...
            >
                {groups.flatMap((group) => (
                    <SelectGroup label={group || "Views"} key={`group_${group}`}>
                        {collections
                ...
                {ungroupedCollections && (
                    <SelectGroup label={"Views"}>
                        {ungroupedCollections.map((collection) => {
                            return (
                                <SelectItem
                                    key={collection.id ?? collection.path}
                                    value={collection.id ?? collection.path}
                                >
                                    <div className="flex flex-row">
                                        <IconForView collectionOrView={collection} />
            ...
            <FieldCaption>You can only edit the reference collection upon field creation.</FieldCaption>
        </>
    );
}
