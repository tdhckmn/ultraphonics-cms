...
export function BlockPropertyField({
    ...
    const { values, setFieldValue } = useFormex<ArrayProperty>();
    ...
    const onPropertyChanged = ({ id, namespace, property }: OnPropertyChangedParams) => {
        if (!id) throw Error();
    ...
    const selectedProperty = selectedPropertyFullId
        ? getIn(values.oneOf?.properties, selectedPropertyFullId.replaceAll(".", ".properties."))
        : undefined;
    ...
    const addChildButton = (
        <Button autoFocus color="primary" onClick={() => setPropertyDialogOpen(true)} startIcon={<AddIcon />}>
        ...
        <>
            <div className={"col-span-12"}>
                <div className={"flex justify-between items-end mt-8 mb-4"}>
                    <Typography variant={"subtitle2"}>Properties in this block</Typography>
                    {addChildButton}
                </div>
                <Paper className="p-2 pl-8">
                    <PropertyTree
                        ...
                        onPropertyMove={disabled ? undefined : onPropertyMove}
                    />
